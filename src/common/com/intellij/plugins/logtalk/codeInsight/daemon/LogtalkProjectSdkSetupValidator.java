 ...
 */
package com.intellij.plugins.logtalk.codeInsight.daemon;
...
import com.intellij.plugins.logtalk.model.LogtalkStdTypesFileModel;
...
import static com.intellij.plugins.logtalk.codeInsight.daemon.SdkValidationResult.*;

public class LogtalkProjectSdkSetupValidator extends JavaProjectSdkSetupValidator {

  @Override
  public boolean isApplicableFor(@NotNull Project project, @NotNull VirtualFile file) {
    final PsiFile psiFile = PsiManager.getInstance(project).findFile(file);
    return psiFile != null && psiFile.getLanguage().isKindOf(LogtalkLanguage.INSTANCE);
  }
    ...
    if (result != null) {
      switch (result) {
        ...
        case MULTIPLE_ROOTS_FOUND:
          return LogtalkBundle.message("sdk.roots.multiple");
        case NO_VALID_SDK_ROOTS_FOUND:
          return LogtalkBundle.message("sdk.roots.no.valid.root");
      }
    }
  ...
  private boolean hasNoValidRoots(List<VirtualFile> roots) {
    return roots.stream().noneMatch(root -> root.findChild(LogtalkStdTypesFileModel.STD_TYPES_HX) != null);
  }
    ...
    if (module != null && !module.isDisposed()) {
      ...
      if (sdk != null) {
        ...
        final VirtualFile stdRoot = getDistinctRootsStream(sdk)
          .filter(root -> root.findChild(LogtalkStdTypesFileModel.STD_TYPES_HX) != null)
          .findFirst()
          .orElse(null);
