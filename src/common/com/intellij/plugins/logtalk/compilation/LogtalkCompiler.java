...
import com.intellij.plugins.logtalk.runner.debugger.LogtalkDebugRunner;
import com.intellij.plugins.logtalk.tests.runner.LogtalkTestsConfiguration;
...
public class LogtalkCompiler implements FileProcessingCompiler {
  private static final Logger LOG = Logger.getInstance("#com.intellij.plugins.logtalk.compilation.LogtalkCompiler");
  ...
  */
  ...
  public String getDescription() {
    return LogtalkBundle.message("logtalk.compiler.description");
  }
  ...
  public ProcessingItem[] getProcessingItems(CompileContext context) {
    final List<ProcessingItem> itemList = new ArrayList<>();
    ...
    for (final Module module : scope.getAffectedModules()) {
      ...
      if (changeSetMap.get(module) != null) { // was built at least once, in past ...
        ...
        Collection<VirtualFile> vFileCollection = FileTypeIndex
          .getFiles(LogtalkFileType.HAXE_FILE_TYPE, module.getModuleWithDependenciesScope());
  ...
  private static ProcessingItem[] run(CompileContext context,
    ...
    if (module == null) {
      context.addMessage(CompilerMessageCategory.ERROR,
                         LogtalkBundle.message("no.module.for.run.configuration", configuration.getName()), null, -1, -1);
      return ProcessingItem.EMPTY_ARRAY;
    }
    LogtalkCommonCompilerUtil.CompilationContext compilationContext = createCompilationContext(context, module, configuration);
  ...
  private static boolean compileModule(final CompileContext context,
                                       Module module,
                                       @NotNull final LogtalkCommonCompilerUtil.CompilationContext compilationContext) {
    ...
    */
    ...
    if (!ModuleUtil.getModuleType(module).equals(LogtalkModuleType.getInstance())) {
      return true;
    }

    boolean compiled = LogtalkCommonCompilerUtil.compile(compilationContext);
  ...
  public static LogtalkCommonCompilerUtil.CompilationContext createDummyCompilationContext(final Module module) {
  ...
  private static LogtalkCommonCompilerUtil.CompilationContext createCompilationContext(final CompileContext context,
                                                                                    final Module module,
                                                                                    ModuleBasedConfiguration configuration) {
    ...
    final boolean isDebug = ExecutorRegistry.getInstance()
      .isStarting(context.getProject(), DefaultDebugExecutor.EXECUTOR_ID, LogtalkDebugRunner.HAXE_DEBUG_RUNNER_ID);
    final ModuleRootManager moduleRootManager = ModuleRootManager.getInstance(module);
    final Sdk sdk = moduleRootManager.getSdk();
    if (sdk == null) {
      context.addMessage(CompilerMessageCategory.ERROR, LogtalkBundle.message("no.sdk.for.module", module.getName()), null, -1, -1);
      return null;
    }

    LogtalkTestsConfiguration haxeTestsConfiguration = null;

    if(configuration != null && configuration instanceof LogtalkTestsConfiguration) {
      haxeTestsConfiguration = (LogtalkTestsConfiguration)configuration;
    }

    final LogtalkTestsConfiguration finalLogtalkTestsConfiguration = haxeTestsConfiguration;

    return new LogtalkCommonCompilerUtil.CompilationContext() {
      private String myErrorRoot;

      @Override
      public LogtalkSdkAdditionalDataBase getLogtalkSdkData() {
        return LogtalkSdkUtilBase.getSdkData(module);
      }
      ...
      public String getCompilationClass() {
        return getIsTestBuild() ? finalLogtalkTestsConfiguration.getRunnerClass() : getModuleSettings().getMainClass();
      }
      ...
      public Boolean getIsTestBuild() {
        return finalLogtalkTestsConfiguration != null;
      }
      ...
      public String getLogtalklibPath() {
        ...
        return data instanceof LogtalkSdkAdditionalDataBase ? ((LogtalkSdkAdditionalDataBase)data).getLogtalklibPath() : null;
      }

      @Override
      public String getNekoBinPath() {
        SdkAdditionalData data = sdk.getSdkAdditionalData();
        return data instanceof  LogtalkSdkAdditionalDataBase ? ((LogtalkSdkAdditionalDataBase)data).getNekoBinPath(): null;
      }
      ...
      public String getModuleDefaultCompileOutputPath() {
        // This is probably the right way to do it, *if* we actually had a CompilerModuleExtension for Logtalk.
      ...
      public void handleOutput(String[] lines) {
        LogtalkCompilerUtil.fillContext(context, getErrorRoot(), lines);
      }

      @Override
      public LogtalkTarget getLogtalkTarget() {
        //actually only neko target is supported for tests
        return getIsTestBuild() ? LogtalkTarget.NEKO : getModuleSettings().getLogtalkTarget();
      }

      private String getFileNameWithCurrentExtension(String fileName) {
        if (getLogtalkTarget() != null) {
          return getLogtalkTarget().getTargetFileNameWithExtension(FileUtil.getNameWithoutExtension(fileName));
        }
        return fileName;
      }
