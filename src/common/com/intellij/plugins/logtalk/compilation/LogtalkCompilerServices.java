...
import com.intellij.plugins.logtalk.config.LogtalkProjectSettings;
import com.intellij.plugins.logtalk.ide.completion.LogtalkCompilerCompletionItem;
...
import com.intellij.plugins.logtalk.util.LogtalkDebugTimeLog;
import com.intellij.plugins.logtalk.util.LogtalkHelpUtil;
 ...
 *
 * Use the Logtalk compiler to help the IDE.  The Logtalk compiler has several
 * functions that are useful, such as type identification, find usages,
 * completion (see LogtalkCompilerCompletionContributor), and others.
 */
public class LogtalkCompilerServices {

    static final LogtalkDebugLogger LOG = LogtalkDebugLogger.getInstance("#com.intellij.plugins.logtalk.compilation.LogtalkCompilerServices");
    static {      // Take this out when finished debugging.
        LOG.setLevel(org.apache.log4j.Level.DEBUG);
    }

    // Cache to keep the openFL display args (read from the project.xml).
    static LogtalkCompilerProjectCache openFLDisplayArguments = new LogtalkCompilerProjectCache();
    ...
    private LogtalkCompilerUtil.ErrorNotifier myErrorNotifier = null;


    public LogtalkCompilerServices(@Nullable LogtalkCompilerUtil.ErrorNotifier errorNotifier) {
        myErrorNotifier = errorNotifier;
    }

    public List<String> getLimeProjectConfiguration(Module module, @Nullable LogtalkDebugTimeLog timeLog) {
        return getLimeProjectConfiguration(module, false, timeLog);
    }

    public List<String> getLimeProjectConfiguration(Module module, boolean useDebugConfig, @Nullable LogtalkDebugTimeLog timeLog) {
        LogtalkModuleSettings moduleSettings = LogtalkModuleSettings.getInstance(module);
        String targetFlag = moduleSettings.getOpenFLTarget().getTargetFlag();

        String projectFilePath = moduleSettings.getOpenFLPath();
        VirtualFile projectFile = LogtalkCompilerUtil.verifyProjectFile(module, "OpenFL", projectFilePath, myErrorNotifier);
        if (null == projectFile) {
            // todo ???
        }
        VirtualFile compileRoot = LogtalkCompilerUtil.findCompileRoot(module);
        ...
        if (compilerArgsFromProjectFile == null) {
            ...
            limeArguments.add(LogtalklibCommandUtils.getLogtalklibPath(module));
            ...
            LogtalkCompilerUtil.runInterruptibleCompileProcess(limeArguments, false,
                                                               compileRoot, LogtalkSdkUtilBase.getSdkData(module),
                                                               stdout, null, timeLog);
    ...
    @NotNull
    public List<LogtalkCompilerCompletionItem> getPossibleCompletions(@NotNull PsiFile file,
                                                                      @NotNull PsiElement element,
                                                                      @NotNull Editor editor) {

        LogtalkDebugTimeLog timeLog = LogtalkDebugTimeLog.startNew("LogtalkCompilerCompletionContributor",
                                                             LogtalkDebugTimeLog.Since.StartAndPrevious);
        List<LogtalkCompilerCompletionItem> completions = LogtalkCompilerCompletionItem.EMPTY_LIST;

        try {
            Project project = file.getProject();
            VirtualFile vfile = file.getVirtualFile();
            VirtualFile compileRoot = LogtalkCompilerUtil.findCompileRoot(file);
            ...
            //Make sure module is Logtalk Module
            if (moduleForFile != null
                && ModuleUtil.getModuleType(moduleForFile).equals(LogtalkModuleType.getInstance())) {
                //Get module settings
                LogtalkModuleSettings moduleSettings = LogtalkModuleSettings.getInstance(moduleForFile);
                LogtalkConfiguration buildConfig = moduleSettings.getBuildConfiguration();
                VirtualFile projectFile = null;
                switch (buildConfig) {
                    case HXML:
                        projectFile = LogtalkCompilerUtil
                          .verifyProjectFile(moduleForFile, "HXML", moduleSettings.getHxmlPath(), myErrorNotifier);
                        if (null == projectFile) {
                            break;
                        }

                        commandLineArguments.add(LogtalkHelpUtil.getLogtalkPath(moduleForFile));
                    ...
                    case NMML:
                        projectFile = LogtalkCompilerUtil
                          .verifyProjectFile(moduleForFile, "NMML", moduleSettings.getNmmlPath(), myErrorNotifier);
                        if (null == projectFile) {
                            break;
                        }
    ...
    protected void advertiseError(String message) {
        LogtalkCompilerUtil.advertiseError(message, myErrorNotifier);
    }
    ...
    private int recalculateFileOffset(@NotNull PsiFile file, @NotNull PsiElement position, Editor editor) {
        ...
        // CRLF - then we have to shift an offset so Logtalk compiler could get proper offset
    ...
    @NotNull
    private List<LogtalkCompilerCompletionItem> collectCompletionsFromCompiler(@NotNull PsiFile file,
                                                                            @NotNull PsiElement element,
                                                                            @NotNull Editor editor,
                                                                            ArrayList<String> commandLineArguments,
                                                                            LogtalkDebugTimeLog timeLog) {
        ...
        // doesn't have access to the correct sources.  If the logtalk compiler is version 3.4 or
        ...
        LogtalkProjectSettings settings = LogtalkProjectSettings.getInstance(project);
        ...
        List<String> stdout = new ArrayList<String>();
        int status = LogtalkCompilerUtil.runInterruptibleCompileProcess(commandLineArguments, false,
                                                                     LogtalkCompilerUtil.findCompileRoot(file),
                                                                     LogtalkSdkUtilBase.getSdkData(moduleForFile),
    ...
    @NotNull
    private List<LogtalkCompilerCompletionItem> collectCompletionsFromNME(@NotNull PsiFile file,
                                                                       @NotNull PsiElement element,
                                                                       @NotNull Editor editor,
                                                                       ArrayList<String> commandLineArguments,
                                                                       LogtalkDebugTimeLog timeLog) {
        ...
        // To get completions out of the logtalk compiler when NME calls it, we have to
        ...
        File tempFile = null;
        try {
            tempFile = File.createTempFile("LogtalkProjectWrapper", ".nmml");
            FileWriter writer = new FileWriter(tempFile);
            writer.write(xml.toString());
            writer.close();

            commandLineArguments.add(LogtalklibCommandUtils.getLogtalklibPath(moduleForFile));
    ...
    @NotNull
    private List<LogtalkCompilerCompletionItem> handleCompilerError(Project project, List<String> stderr) {
    ...
    private void reportErrors(Project project, List<String> errors) {
        for (String error : errors) {
            ...
            LogtalkCompilerError compilerError = LogtalkCompilerError.create(projectPath, error);
    ...
    private List<LogtalkCompilerCompletionItem> parseCompletionFromXml(Project project, List<String> stderr) {
        List<LogtalkCompilerCompletionItem> completions = new ArrayList<LogtalkCompilerCompletionItem>();
        try {
            ...
            if ("list".equals(rootTag.getName())) {
                ...
                for (XmlTag xmlTag : xmlTags) {
                    ...
                    LogtalkCompilerCompletionItem item = new LogtalkCompilerCompletionItem(name);
                ...
                LogtalkCompilerCompletionItem item = new LogtalkCompilerCompletionItem("Type");
            ...
            advertiseError("Logtalk compiler completion canceled.");  // TODO: Externalize string.
            LOG.debug("Logtalk compiler completion canceled.", e);
            throw e;
        }
        return completions;
    }
    ...
    //https://github.com/LogtalkIDE/HIDE/blob/master/src/core/FunctionParametersHelper.hx#L193
    public void parseFunctionParams(String type, LogtalkCompilerCompletionItem item)
    {
