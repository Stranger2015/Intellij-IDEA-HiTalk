...
public class LogtalkSdkUtil extends LogtalkSdkUtilBase {
  private static final Logger LOG = Logger.getInstance("#com.intellij.plugins.logtalk.config.sdk.LogtalkSdkUtil");
  private static final Pattern VERSION_MATCHER = Pattern.compile("(\\d+(\\.\\d+)+)");

  @Nullable
  public static LogtalkSdkData testLogtalkSdk(String path) {
    ...
    try {
      ...
      if (output.getExitCode() != 0) {
        LOG.error("Logtalk compiler exited with invalid exit code: " + output.getExitCode());
        return null;
      }
      ...
      final LogtalkSdkData haxeSdkData = new LogtalkSdkData(path, haxeVersion);
      haxeSdkData.setLogtalklibPath(getLogtalklibPathByFolderPath(path));
  ...
  public static String suggestHomePath() {
    ...
    if (result == null && !SystemInfo.isWindows) {
      final String candidate = "/usr/lib/logtalk";
