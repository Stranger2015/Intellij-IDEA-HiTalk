...
public class IfConditionFixer implements Fixer {
  ...
  public void apply(Editor editor, LogtalkSmartEnterProcessor processor, PsiElement psiElement) throws IncorrectOperationException {
    if (!(psiElement instanceof LogtalkIfStatement)) {
      return;
    }

    LogtalkIfStatement ifStatement = (LogtalkIfStatement)psiElement;

    if (null != ifStatement && ifStatement.getBlockStatementList().size() > 0) {
      return;
    }

    if (ifStatement.getNode().findChildByType(LogtalkTokenTypes.PLPAREN) == null) {
      int offset = ifStatement.getFirstChild().getTextRange().getEndOffset();
      editor.getDocument().insertString(offset, " () {\n}");
      editor.getCaretModel().moveToOffset(offset + 2);
      processor.setSkipEnter(true);
    }
    else if (null != ifStatement.getExpressionList() && ifStatement.getNode().findChildByType(LogtalkTokenTypes.PLCURLY) == null) {
      int offset = ifStatement.getNode().findChildByType(LogtalkTokenTypes.PRPAREN).getTextRange().getEndOffset();
      editor.getDocument().insertString(offset, " {\n\n}");
