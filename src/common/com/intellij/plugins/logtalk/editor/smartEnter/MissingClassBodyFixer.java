 ...
 * Adapted to Logtalk from com.intellij.codeInsight.editorActions.smartEnter.MissingClassBodyFixer
 */
public class MissingClassBodyFixer implements Fixer {
  ...
  public void apply(Editor editor, LogtalkSmartEnterProcessor processor, PsiElement psiElement) throws IncorrectOperationException {
    if (!(psiElement instanceof LogtalkClass)) {
      return;
    }

    LogtalkClass haxeClass = (LogtalkClass)psiElement;

    ASTNode node = haxeClass.getNode().findChildByType(LogtalkTokenTypes.PLCURLY);

    LogtalkComponentName haxeClassComponentName = haxeClass.getComponentName();
