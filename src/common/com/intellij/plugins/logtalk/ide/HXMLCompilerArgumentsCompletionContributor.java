...
import com.intellij.plugins.haxe.util.LogtalkHelpUtil;
...
public class HXMLCompilerArgumentsCompletionContributor extends CompletionContributor {
  ...
  private List<String> getStrings(ArrayList<String> commandLine) {
    ...
    commandLine.add(LogtalkHelpUtil.getLogtalkPath(module));
    commandLine.add("--help");

    List<String> strings = LogtalklibCommandUtils.getProcessStderr(commandLine, LogtalkSdkUtilBase.getSdkData(module));
    if (strings.size() > 0) {
      strings.remove(0);
    }
    return strings;
