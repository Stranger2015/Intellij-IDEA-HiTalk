...
public class LogtalkFindUsagesHandlerFactory extends FindUsagesHandlerFactory {

  static FindUsagesProvider haxeProvider;

  static FindUsagesProvider getLogtalkProvider() {
    if (null == haxeProvider) {
      haxeProvider = LanguageFindUsages.INSTANCE.forLanguage(LogtalkLanguage.INSTANCE); // Find and use the provider specified in plugin.xml.
    }
    return haxeProvider;
  }


  LogtalkFindUsagesHandlerFactory(Project project) {
    super();
  }
  ...
  public boolean canFindUsages(@NotNull final PsiElement element) {
    ...
    if (element.getLanguage() != LogtalkLanguage.INSTANCE
    ||  !getLogtalkProvider().canFindUsagesFor(element)) {
      return false;
    }
  ...
  public FindUsagesHandler createFindUsagesHandler(@NotNull final PsiElement element, final boolean forHighlightUsages) {
    if (canFindUsages(element)) {
      PsiElement target = LogtalkFindUsagesUtil.getTargetElement(element);
      ...
      if (!forHighlightUsages) {
        ...
        if (target instanceof LogtalkMethod) {
          PsiMethod[] supers = ((LogtalkMethod)target).findSuperMethods();
          if (supers.length != 0) {
            String chosen = askWhetherToSearchForOverridingMethods(target);
            if (CURRENT_CLASS == chosen)    { return new LogtalkFindUsagesHandler(target); }
            else if (BASE_CLASS == chosen)  { return new LogtalkFindUsagesHandler(supers[supers.length - 1]); }
            else /* ANCESTOR_CLASSES */     { return new LogtalkFindUsagesHandler(target, supers); }
          }
        }
      }

      return new LogtalkFindUsagesHandler(target != null ? target : element);
    }
