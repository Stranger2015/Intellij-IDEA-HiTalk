 ...
 */
public class LogtalkTypeAddImportIntentionAction implements HintAction, QuestionAction, LocalQuickFix {
  private final List<LogtalkComponent> candidates;
  private final PsiElement myType;
  private Editor myEditor;

  public LogtalkTypeAddImportIntentionAction(@NotNull PsiElement type, @NotNull List<LogtalkComponent> components) {
    myType = type;
  ...
  public String getText() {
    if (candidates.size() > 1) {
      final LogtalkClass haxeClass = (LogtalkClass)candidates.iterator().next();
      return LogtalkBundle.message("add.import.multiple.candidates", haxeClass.getQualifiedName());
    }
  ...
  public void invoke(@NotNull final Project project, final Editor editor, PsiFile file) throws IncorrectOperationException {
    if (candidates.size() > 1) {
      NavigationUtil.getPsiElementPopup(
        ...
        LogtalkBundle.message("choose.class.to.import.title"),
  ...
  private void doImport(final Editor editor, final PsiElement component) {
    new WriteCommandAction(myType.getProject(), myType.getContainingFile()) {
      ...
      protected void run(Result result) {
        LogtalkAddImportHelper.addImport(((LogtalkClass)component).getQualifiedName(), myType.getContainingFile());
      }
    }.execute();
  }
