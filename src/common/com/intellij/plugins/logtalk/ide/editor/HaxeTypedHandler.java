 ...
 */
package com.intellij.plugins.logtalk.ide.editor;
...
public class LogtalkTypedHandler extends TypedHandlerDelegate {
  private static final TokenSet INVALID_INSIDE_REFERENCE =
    TokenSet.create(LogtalkTokenTypes.OSEMI, LogtalkTokenTypes.PLCURLY, LogtalkTokenTypes.PRCURLY);
  ...
  public Result beforeCharTyped(char c,
    ...
    if (c == '>') {
      if (CodeInsightSettings.getInstance().AUTOINSERT_PAIR_BRACKET &&
          JavaTypedHandler.handleJavaGT(editor, LogtalkTokenTypes.OLESS, LogtalkTokenTypes.OGREATER, INVALID_INSIDE_REFERENCE)) {
        return Result.STOP;
      }
    }
  ...
  private static boolean checkAfterTypeOrComponentName(PsiFile file, int offset) {
    ...
    return PsiTreeUtil.getParentOfType(toCheck, LogtalkType.class, LogtalkComponentName.class) != null;
  }

  private static boolean checkAfterDollarInString(PsiFile file, int offset) {
    PsiElement at = file.findElementAt(offset - 1);
    final String text = at != null ? at.getText() : "";
    return text.endsWith("$") && PsiTreeUtil.getParentOfType(at, LogtalkPsiCompositeElement.class) != null;
  }
