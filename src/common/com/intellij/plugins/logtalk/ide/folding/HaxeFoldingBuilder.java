...
import static com.intellij.plugins.haxe.lang.lexer.LogtalkTokenTypeSets.*;
...
public class LogtalkFoldingBuilder implements FoldingBuilder {
  ...
  private static final Key<Boolean> REGION_DEFINITION_KEY = new Key<>("LogtalkRegionDefinition");
  ...
  private static String stripComment(IElementType elementType, String text) {
    if (elementType == MSL_COMMENT) {
      return strip(text, LogtalkCommenter.SINGLE_LINE_PREFIX, null);
    }
    if (elementType == MML_COMMENT) {
      text = LogtalkStringUtil.terminateAt(text, '\n');
      return strip(text.trim(), LogtalkCommenter.BLOCK_COMMENT_PREFIX, LogtalkCommenter.BLOCK_COMMENT_SUFFIX);
    }
    if (elementType == DOC_COMMENT) {
      text = LogtalkStringUtil.terminateAt(text, '\n');
      return strip(text, LogtalkCommenter.DOC_COMMENT_PREFIX, LogtalkCommenter.DOC_COMMENT_SUFFIX);
    }
    return text.trim();
  }
  ...
  private static String strip(@NotNull String text, @Nullable String prefix, @Nullable String suffix) {
    ...
    if (null != prefix) stripped = LogtalkStringUtil.stripPrefix(stripped, prefix);
    if (null != suffix) stripped = LogtalkStringUtil.stripSuffix(stripped, suffix);
  ...
  public boolean isCollapsedByDefault(@NotNull ASTNode node) {
    ...
    //final LogtalkFoldingSettings haxeSettings = LogtalkFoldingSettings.getInstance();

    if (psiElement instanceof LogtalkImportStatement || psiElement instanceof LogtalkUsingStatement) {
