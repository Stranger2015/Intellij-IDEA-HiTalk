 ...
 */
public class LogtalkSpacingProcessor {
  private final ASTNode myNode;
  private final CommonCodeStyleSettings mySettings;
  private final LogtalkCodeStyleSettings myLogtalkCodeStyleSettings;

  public LogtalkSpacingProcessor(ASTNode node, CommonCodeStyleSettings settings, LogtalkCodeStyleSettings haxeCodeStyleSettings) {
    myNode = node;
    mySettings = settings;
    myLogtalkCodeStyleSettings = haxeCodeStyleSettings;
  }
  ...
  public Spacing getSpacing(Block child1, Block child2) {
    ...
    if (type1 == OCOLON && elementType == TYPE_TAG) {
      return addSingleSpaceIf(myLogtalkCodeStyleSettings.SPACE_AFTER_TYPE_REFERENCE_COLON);
    }

    if (type2 == TYPE_TAG) {
      return addSingleSpaceIf(myLogtalkCodeStyleSettings.SPACE_BEFORE_TYPE_REFERENCE_COLON);
    }

    if (type1 == OARROW || type2 == OARROW) {
      return addSingleSpaceIf(myLogtalkCodeStyleSettings.SPACE_AROUND_ARROW);
    }
