...
import com.intellij.plugins.haxe.lang.psi.LogtalkNewExpression;
import com.intellij.plugins.haxe.lang.psi.LogtalkReferenceExpression;
import com.intellij.plugins.haxe.lang.psi.LogtalkSuperExpression;
 ...
 */
public class LogtalkCallReferenceProcessor implements CallReferenceProcessor {
  private static final Logger LOG = Logger.getInstance("#com.intellij.plugins.logtalk.ide.hierarchy.call.LogtalkCallReferenceProcessor");

  public static class CallData extends JavaCallHierarchyData {
    LogtalkHierarchyTimeoutHandler myTimeoutHandler;
    ...
    public CallData(PsiClass originalClass,
                                 ...
                                 Project project,
                                 LogtalkHierarchyTimeoutHandler timeoutHandler) {
    ...
    public LogtalkHierarchyTimeoutHandler getTimeoutHandler() {
  ...
  public boolean process(@NotNull PsiReference reference, @NotNull JavaCallHierarchyData jchdata) {
    ...
    LogtalkHierarchyTimeoutHandler timeoutHandler = data.getTimeoutHandler();
    ...
    if (reference instanceof LogtalkReferenceExpression) {
      final PsiElement qualifierElement = ((LogtalkReferenceExpression)reference).getQualifier();
      final LogtalkReferenceExpression qualifier = (LogtalkReferenceExpression) qualifierElement;
      if (qualifier instanceof LogtalkSuperExpression) { // filter super.foo() call inside foo() and similar cases (bug 8411)
      ...
      if (parent instanceof LogtalkNewExpression) {
        if (((LogtalkNewExpression)parent).getType().getReferenceExpression() != reference) {
          return true;
        }
      }
      // If the reference isn't the primary expression of an anonymous class, then keep looking?
        ...
        //      calling for the base reference (the first child of the class).  The logtalk
