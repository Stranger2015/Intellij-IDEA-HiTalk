...
import com.intellij.plugins.haxe.ide.hierarchy.LogtalkHierarchyNodeDescriptor;
...
import com.intellij.plugins.logtalk.ide.hierarchy.LogtalkHierarchyNodeDescriptor;
 ...
 */
public final class LogtalkMethodHierarchyNodeDescriptor extends LogtalkHierarchyNodeDescriptor {

  //
  // Lifted, Lock, Stock, and Barrel from MethodHierarchyNodeDescriptor.  Then
  // hacked on for Logtalk purposes.  The original MethodHierarchyNodeDescriptor,
  // while not final, had methods that were, and we needed to override them.
  //

  private Icon myRawIcon;
  private Icon myStateIcon;
  private LogtalkMethodHierarchyTreeStructure myTreeStructure;

  public LogtalkMethodHierarchyNodeDescriptor(final Project project,
                                       final HierarchyNodeDescriptor parentDescriptor,
                                       final PsiElement aClass,
                                       final boolean isBase,
                                       final LogtalkMethodHierarchyTreeStructure treeStructure
  ) {
    super(project, parentDescriptor, aClass, isBase);
    myTreeStructure = treeStructure;
  }

  public final void setTreeStructure(final LogtalkMethodHierarchyTreeStructure treeStructure) {
    myTreeStructure = treeStructure;
  }

  PsiMethod getMethod(final PsiClass aClass, final boolean checkBases) {
    return LogtalkHierarchyUtils.findBaseMethodInClass(myTreeStructure.getBaseMethod(), aClass, checkBases);
  }
  ...
  */
  public final PsiElement getTargetElement() {
    final LogtalkClass theLogtalkClass = getLogtalkClass();
    if ((null == theLogtalkClass) || (! theLogtalkClass.isValid())) return null;
    final PsiMethod method = getMethod(theLogtalkClass, false);
    if (method != null) return method;
    return theLogtalkClass;
  }
  ...
  public final boolean update() {
    ...
    final PsiElement psiClass = getLogtalkClass();
  ...
  private Icon calculateState(final PsiClass psiClass) {
    ...
    //  because Logtalk doesn't have Java's idea of abstract methods.
    //  (In Logtalk, abstract refers to /adding/ to an existing class or type.)
    //}
