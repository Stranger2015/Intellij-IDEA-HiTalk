 ...
 */
public class LogtalkTypeHierarchyProvider implements HierarchyProvider {
  private static final Logger LOG = Logger.getInstance("#com.intellij.ide.hierarchy.type.LogtalkTypeHierarchyProvider");

  static {
    LOG.info("Loaded LogtalkTypeHierarchyProvider");
  ...
  public PsiElement getTarget(@NotNull DataContext context) {
    LogtalkClass clazz = LogtalkHierarchyUtils.getContainingClass(
                                  context, false /* No anonymous classes */ );
    if (null == clazz) {
      LogtalkFile f = LogtalkHierarchyUtils.getContainingFile(context);
      if (f != null) {
        LogtalkClass[] classes = LogtalkHierarchyUtils.getClassList(f);
  ...
  public HierarchyBrowser createHierarchyBrowser(PsiElement element) {
    return new LogtalkTypeHierarchyBrowser(element.getProject(), (PsiClass) element);
  }

  @Override
  public void browserActivated(@NotNull HierarchyBrowser hierarchyBrowser) {
    final LogtalkTypeHierarchyBrowser browser = (LogtalkTypeHierarchyBrowser) hierarchyBrowser;
