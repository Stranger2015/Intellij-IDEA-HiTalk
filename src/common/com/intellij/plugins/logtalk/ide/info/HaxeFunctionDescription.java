...
public class LogtalkFunctionDescription {
  private static final TextRange ZERO_TEXT_RANGE = new TextRange(0, 0);
  private static final String PARAMETERS_DELIMITER = ", ";

  private final LogtalkParameterDescription[] parameters;

  private final String description;
  private final TextRange[] parametersTextRange;

  LogtalkFunctionDescription(@Nullable LogtalkParameterDescription[] parameters) {
  ...
  public LogtalkParameterDescription[] getParameters() {
    return parameters;
  }
  ...
  private String compilePresentableDescription() {
    ...
    if (parameters.length == 0) {
      return LogtalkBundle.message("logtalk.parameter.info.helper.no.parameters");
    }

    for (int i = 0; i < parameters.length; i++) {
      LogtalkParameterDescription parameter = parameters[i];
