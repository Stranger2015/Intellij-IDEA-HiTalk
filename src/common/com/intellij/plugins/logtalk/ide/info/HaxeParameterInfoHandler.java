 ...
 */
public class LogtalkParameterInfoHandler implements ParameterInfoHandler<PsiElement, LogtalkFunctionDescription> {
  ...
  public Object[] getParametersForLookup(@NotNull LookupElement item, ParameterInfoContext context) {
    ...
    if (object instanceof PsiElement) {
      ...
      final LogtalkComponentType type = LogtalkComponentType.typeOf(element.getParent());
  ...
  @Nullable
  @Override
  public Object[] getParametersForDocumentation(@NotNull LogtalkFunctionDescription description, ParameterInfoContext context) {
  ...
  private PsiElement findCallOrNewExpressionUnderCaret(@NotNull ParameterInfoContext context) {
    ...
    return PsiTreeUtil.getParentOfType(place, LogtalkCallExpression.class, LogtalkNewExpression.class);
  }

  @Override
  public void showParameterInfo(@NotNull PsiElement element, @NotNull CreateParameterInfoContext context) {
    final LogtalkFunctionDescription functionDescription = getParametersDescriptions(element);
  ...
  @Nullable
  private LogtalkFunctionDescription getParametersDescriptions(PsiElement element) {
    if (element instanceof LogtalkCallExpression) {
      return LogtalkFunctionDescriptionBuilder.buildForMethod((LogtalkCallExpression)element);
    }
    else if (element instanceof LogtalkNewExpression) {
      return LogtalkFunctionDescriptionBuilder.buildForConstructor((LogtalkNewExpression)element);
    }
  ...
  public void updateParameterInfo(@NotNull PsiElement place, @NotNull UpdateParameterInfoContext context) {
    ...
    if (owner != PsiTreeUtil.getParentOfType(place, LogtalkCallExpression.class, LogtalkNewExpression.class)) {
      context.removeHint();
      return;
    }
  ...
  private int getArgumentIndex(@NotNull PsiElement owner, @NotNull PsiElement place) {
    ...
    final LogtalkFunctionDescription functionDescription = getParametersDescriptions(owner);

    if (functionDescription == null || functionDescription.getParameters().length == 0) return argumentIndex;

    final List<LogtalkExpression> argumentsList = getArgumentsList(owner);
    final LogtalkParameterDescription[] functionParameters = functionDescription.getParameters();
  ...
  private int getArgumentIndexUnderCaret(@NotNull PsiElement place, List<LogtalkExpression> expressionList) {
    LogtalkExpression expression = getExpressionAtPlace(place, expressionList);
    ...
    if (expression != null) {
      ...
      if (tokenText.equals(LogtalkTokenTypes.PRPAREN.toString())) {
  ...
  private LogtalkExpression getExpressionAtPlace(@NotNull PsiElement place, final List<LogtalkExpression> expressionList) {
    return (LogtalkExpression)PsiTreeUtil.findFirstParent(place, new Condition<PsiElement>() {
      @Override
      public boolean value(PsiElement element) {
        return element instanceof LogtalkExpression && expressionList.indexOf(element) >= 0;
      }
    });
  }

  private int getExpressionIndexAtPlace(PsiElement place, List<LogtalkExpression> list) {
  ...
  private int getExpressionIndexBeforeRightParen(List<LogtalkExpression> list) {
    final int listSize = list.size();
    final PsiElement commaExpression = (UsefulPsiTreeUtil.getNextSiblingSkippingCondition(list.get(listSize-1), new Condition<PsiElement>() {
      @Override
      public boolean value(PsiElement element) {
        return !(element instanceof LogtalkPsiToken && element.getText().equals(LogtalkTokenTypes.OCOMMA.toString()));
      }
    }, false));
  ...
  private int interpolateArgumentIndexToParameterIndex(List<LogtalkExpression> arguments,
                                                       LogtalkParameterDescription[] parameters,
    ...
    for (int parameterIndex = 0; parameterIndex < parameterCount; parameterIndex++) {
      ...
      final LogtalkParameterDescription parameter = parameters[parameterIndex];
      ...
      final LogtalkExpression argument = arguments.get(argumentIndex);

      final ResultHolder parameterType = parameter.getResultHolder();
      final ResultHolder argumentType = LogtalkTypeResolver.getPsiElementType(argument);
  ...
  private List<LogtalkExpression> getArgumentsList(@NotNull PsiElement element) {
    if (element instanceof LogtalkNewExpression) {
      return ((LogtalkNewExpression)element).getExpressionList();
    }
    else if (element instanceof LogtalkCallExpression) {
      LogtalkExpressionList expressionList = ((LogtalkCallExpression)element).getExpressionList();
  ...
  @Override
  public void updateUI(@Nullable LogtalkFunctionDescription description, @NotNull ParameterInfoUIContext context) {
    if (description == null) {
