 ...
 */
public class LogtalkPreprocessorInspection extends LocalInspectionTool {
  @NotNull
  ...
  public String getDisplayName() {
    return LogtalkBundle.message("logtalk.inspection.preprocessor.symbol.long.name");
  }
  ...
  public String getShortName() {
    return LogtalkBundle.message("logtalk.inspection.preprocessor.symbol.name");
  }
  ...
  public ProblemDescriptor[] checkFile(@NotNull PsiFile file, @NotNull final InspectionManager manager, final boolean isOnTheFly) {
    ...
    while (leaf != null) {
      ...
      if (leafElementType.equals(LogtalkTokenTypes.PPIF)) {
        nodes = new ArrayList<ASTNode>();
        levels.push(nodes);
        nodes.add(leaf);
      }
      else if (leafElementType.equals(LogtalkTokenTypes.PPEND)) {
        nodes.add(leaf);
      ...
      else if (leafElementType.equals(LogtalkTokenTypes.PPELSEIF)) {
        nodes.add(leaf);
      }
      else if (leafElementType.equals(LogtalkTokenTypes.PPELSE)) {
        nodes.add(leaf);
      }
  ...
  private void validateLevel(List<ASTNode> nodes, @NotNull ProblemReporter reporter) {
    ...
    if (node.getElementType() != LogtalkTokenTypes.PPIF) {
      markAllNodes(nodes, "Missing #\bif for this conditional compiler directive.", reporter);
    }
    node = nodes.get(nodes.size()-1);
    if (node.getElementType() != LogtalkTokenTypes.PPEND) {
    ...
    for (ASTNode current : nodes) {
      if (current.getElementType() == LogtalkTokenTypes.PPELSEIF) {
        if (sawElse) {
