 ...
 */
public class LogtalkLibRootDetector extends RootDetector {
  protected LogtalkLibRootDetector(OrderRootType rootType, String presentableRootTypeName) {
      ...
      public boolean visitFile(@NotNull VirtualFile file) {
        ...
        if (file.findChild(".current") != null) {
          for (VirtualFile child : file.getChildren()) {
            if (child.isDirectory() && containsLogtalkFiles(child)) {
              result.add(child);
            }
          }
          return false;
  ...
  private static boolean containsLogtalkFiles(final VirtualFile dir) {
    final VirtualFileVisitor.Result result = VfsUtilCore.visitChildrenRecursively(dir, new VirtualFileVisitor() {
      @NotNull
      @Override
      public Result visitFileEx(@NotNull VirtualFile file) {
        return !file.isDirectory() && LogtalkFileType.DEFAULT_EXTENSION.equalsIgnoreCase(file.getExtension()) ? skipTo(dir) : CONTINUE;
      }
    });
