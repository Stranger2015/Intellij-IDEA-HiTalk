 ...
 */
package com.intellij.plugins.logtalk.ide.refactoring.memberPullUp;
...
import com.intellij.plugins.haxe.lang.psi.impl.AbstractLogtalkPsiClass;
 ...
 */
public class LogtalkPullUpHandler implements RefactoringActionHandler, LogtalkPullUpDialog.Callback, ElementsHandler {
  private static final Logger LOG = Logger.getInstance("#com.intellij.plugins.logtalk.ide.refactoring.memberPullUp.LogtalkPullUpHandler");
  ...
  public void invoke(@NotNull Project project, Editor editor, PsiFile file, DataContext context) {
    ...
    //LogtalkClassDeclaration classDeclaration;
    ...
    while (true) {
      ...
      /*classDeclaration = PsiTreeUtil.getParentOfType(element, LogtalkClassDeclaration.class, false);

      parentElement = null;
      parentElement = PsiTreeUtil.getParentOfType(element, LogtalkVarDeclaration.class, false);
      if (parentElement == null) {
        parentElement = PsiTreeUtil.getParentOfType(element, LogtalkFunctionDeclarationWithAttributes.class, false);
      }*/

      if (element instanceof LogtalkClassDeclaration || element instanceof LogtalkInterfaceDeclaration || element instanceof PsiField || element instanceof PsiMethod) {
  ...
  public void invoke(@NotNull Project project, @NotNull PsiElement[] elements, DataContext context) {
    ...
    if (element instanceof LogtalkClassDeclaration || element instanceof LogtalkInterfaceDeclaration) {
      aClass = (AbstractLogtalkPsiClass)element;
    }
  ...
  private void invoke(Project project, DataContext dataContext, PsiClass psiClass, PsiElement aMember) {
    AbstractLogtalkPsiClass aClass = (AbstractLogtalkPsiClass)psiClass;
    ...
    List<LogtalkType> extendsList = aClass.getLogtalkExtendsList();
    List<LogtalkType> implementsList = aClass.getLogtalkImplementsList();
    if (extendsList.isEmpty() && implementsList.isEmpty()) {
      final AbstractLogtalkPsiClass containingClass = aClass;
    ...
    LogtalkClass haxeClass;
    for (int i = 0; i < extendsList.size(); i++) {
      result = extendsList.get(i).getReferenceExpression().resolveLogtalkClass();
      if (result != null) {
    ...
    for (int i = 0; i < implementsList.size(); i++) {
      result = implementsList.get(i).getReferenceExpression().resolveLogtalkClass();
      if (result != null) {
        haxeClass = result.getLogtalkClass();
        if (haxeClass != null) {
          psiClasses.add(haxeClass);
    ...
    final LogtalkPullUpDialog dialog = new LogtalkPullUpDialog(project, aClass, psiClasses, memberInfoStorage, this);
    dialog.show();
  }

  @Override
  public boolean checkConflicts(final LogtalkPullUpDialog dialog) {
