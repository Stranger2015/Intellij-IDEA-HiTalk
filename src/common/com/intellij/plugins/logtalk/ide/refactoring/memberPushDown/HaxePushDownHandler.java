 ...
 */
package com.intellij.plugins.logtalk.ide.refactoring.memberPushDown;
 ...
 */
public class LogtalkPushDownHandler extends JavaPushDownHandler {
  @Override
  public void invoke(@NotNull Project project, Editor editor, PsiFile file, DataContext context) {
    ...
    while (true) {
      ...
      if (element instanceof LogtalkClassDeclaration || element instanceof LogtalkVarDeclaration || element instanceof LogtalkFunctionDeclarationWithAttributes
        || element instanceof LogtalkFunctionPrototypeDeclarationWithAttributes) {
  ...
  public void invoke(@NotNull Project project, @NotNull PsiElement[] elements, DataContext context) {
    ...
    if (element instanceof LogtalkClassDeclaration) {
      aClass = (LogtalkClassDeclaration) element;
    } else if (element instanceof LogtalkFunctionDeclarationWithAttributes) {
      aClass = ((LogtalkFunctionDeclarationWithAttributes) element).getContainingClass();
      aMember = element;
    } else if (element instanceof LogtalkFunctionPrototypeDeclarationWithAttributes) {
      aClass = ((LogtalkFunctionPrototypeDeclarationWithAttributes) element).getContainingClass();
      aMember = element;
    } else if (element instanceof LogtalkVarDeclaration) {
      aClass = ((LogtalkVarDeclaration)element).getContainingClass();
      aMember = element;
    }
    else {
      return;
    ...
    LogtalkPushDownDialog dialog = new LogtalkPushDownDialog(
      project,
