...
import com.intellij.plugins.logtalk.lang.util.LogtalkConditionalExpression;
...
/**
 * Support Conditional Compilation for the Logtalk language.
 *
 * Created by ebishton on 3/23/17.
 */
public class LogtalkConditionalCompilationLexerSupport {
  static final String CLASS_NAME = new Object(){}.getClass().getEnclosingClass().getName();
  static final LogtalkDebugLogger LOG = LogtalkDebugLogger.getLogger('#' + CLASS_NAME);
  ...
  private static final class Block {
    ...
    private LogtalkConditionalExpression myCondition;
    ...
    public Block(Section s, IElementType el) {
      ...
      switch (myBlockType) {
        case IF:
        case ELSEIF:
          myCondition = new LogtalkConditionalExpression(null);
          break;
        default:
          break;
      }
    ...
    @Nullable
    public LogtalkConditionalExpression getCondition() {
      return myCondition;
    }
  ...
  private class Section {
    ...
    private void setBlockActiveState(Block block) {
      if (block != null && block.getActive() == Block.ActiveState.UNKNOWN) {
        LogtalkConditionalExpression condition = block.getCondition();
  ...
  public LogtalkConditionalCompilationLexerSupport(Project context) {
    reset(context);
  }

  /**
   * Reset conditional compilation support to a pristine state.
   * Called on initialization and when the LogtalkGeneratedLexerWrapper receives a
  ...
  public void conditionEnd() {
    LogtalkConditionalExpression condition = getCurrentBlock().getCondition();
