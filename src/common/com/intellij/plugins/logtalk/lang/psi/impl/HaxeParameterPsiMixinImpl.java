...
import com.intellij.plugins.logtalk.lang.psi.LogtalkParameterPsiMixin;
 ...
 */
public abstract class LogtalkParameterPsiMixinImpl extends AbstractLogtalkNamedComponent implements LogtalkParameterPsiMixin {

  private static final Logger LOG = Logger.getInstance("#com.intellij.plugins.logtalk.lang.psi.impl.LogtalkParameterBase");

  public LogtalkParameterPsiMixinImpl(ASTNode node) {
    super(node);
  }

  public LogtalkParameterPsiMixinImpl(PsiParameter parameter) {
    super(parameter.getNode());
  }

  public LogtalkParameterPsiMixinImpl(LogtalkParameter parameter) {
    super(parameter.getNode());
  }
  ...
  public boolean isVarArgs() {
    // In Logtalk (http://old.haxe.org/doc/cross/reflect), there are no
  ...
  public PsiTypeElement getTypeElement() {
    ...
    //       Need to verify against the Logtalk language spec.
    //              Are there other situations?
    ...
    // PsiTypeElement t = (LogtalkType) PsiTreeUtil.findChildOfType(this, LogtalkType.class);
  ...
  public PsiType getType() {
    // The Logtalk language variable type (int, float, etc.), not the psi token type.
  ...
  public LogtalkModifierList getModifierList() {
    LogtalkModifierList haxePsiModifierList = new LogtalkModifierListImpl(this.getNode());

    // Triplicated code! LogtalkMethodPsiMixinImpl + LogtalkParameterPsiMixinImpl + LogtalkPsiFieldImpl
    if (isStatic()) {
      haxePsiModifierList.setModifierProperty(LogtalkPsiModifier.STATIC, true);
    }

    if (isInline()) {
      haxePsiModifierList.setModifierProperty(LogtalkPsiModifier.INLINE, true);
    }

    if (isPublic()) {
      haxePsiModifierList.setModifierProperty(LogtalkPsiModifier.PUBLIC, true);
    }
    else {
      haxePsiModifierList.setModifierProperty(LogtalkPsiModifier.PRIVATE, true);
    }
    ...
    // E.g. see AbstractLogtalkClassPsi
