 ...
 */
public class LogtalkTypeListPartPsiMixinImpl extends LogtalkPsiCompositeElementImpl implements LogtalkTypeListPartPsiMixin {
  ...
  // holds is LogtalkAnonymousType, which itself derives from LogtalkClass (thus, PsiClass).
  // So, I'd prefer that LogtalkTypePsiMixin implement the class interface.  Then we
  ...
  //
  // The third child type that we can hold is LogtalkFunctionType, which is simply a
  ...
  static Logger LOG = Logger.getInstance("#com.intellij.plugins.logtalk.lang.psi.impl.LogtalkTypeListPartPsiMixinImpl");
  ...
  LogtalkTypeListPartPsiMixinImpl(ASTNode node) {
    super(node);
  }
  ...
  private PsiClass getDelegate() {
    ...
    // lookup (resolveLogtalkClass()), which has a caching algorithm of its own and
    ...
    if (null == myChildClass) {
      ...
      if ( null != node ) {
        ...
        if (LogtalkTokenTypes.TYPE_OR_ANONYMOUS.equals(type)) {

          target = child.getFirstChild();
          IElementType targetType = target.getNode().getElementType();

          if (LogtalkTokenTypes.TYPE.equals(targetType)) {
            myChildClass = new LogtalkClassDelegateForTypeChild((LogtalkType)target);
          } else if (LogtalkTokenTypes.ANONYMOUS_TYPE.equals(targetType)){
            myChildClass = (LogtalkAnonymousType) target;
          } else {
            LOG.debug("Target: " + target.toString());
            LOG.assertTrue(false, "Unexpected token type for child of TYPE_OR_ANONYMOUS");
            myChildClass = AbstractLogtalkPsiClass.EMPTY_FACADE;
          }
        } else if (LogtalkTokenTypes.FUNCTION_TYPE.equals(type)) {
          // FIXME: Temporary Hack to get around an unexpected PSI state #627.
          try {
            myChildClass = (LogtalkAnonymousType) child;
          }
          catch(ClassCastException e) {
            LogtalkDebugLogger.getLogger().warn("Unexpected PSI state.  See issue #627");
            myChildClass = AbstractLogtalkPsiClass.EMPTY_FACADE;
          }
        } else {
          myChildClass = AbstractLogtalkPsiClass.EMPTY_FACADE;
  ...
  public LogtalkModifierList getModifierList() {
    return (LogtalkModifierList) getDelegate().getModifierList();
  }
  ...
  public PsiJavaToken getLBrace() {
    // Casting this is only doable because LogtalkAstFactory
  ...
  // //////////////////////////////////////////////////////////////////////////////////
  //
  // LogtalkTypeListPartForTypeChild
  //
  // //////////////////////////////////////////////////////////////////////////////////
  // //////////////////////////////////////////////////////////////////////////////////

  /**
   * Deals with type references that appear as children to LogtalkTypeOrAnonymous
   * child elements.
   */
  private class LogtalkClassDelegateForTypeChild extends PsiTypeParameterImpl {

    private LogtalkType myChildType = null;

    public LogtalkClassDelegateForTypeChild(@NotNull LogtalkType childType) {
      super(childType.getNode());
      myChildType = childType;
    }

    @NotNull
    protected LogtalkReference getDelegate() {
      return myChildType.getReferenceExpression();
    }

    @Nullable
    protected LogtalkClass getResolvedDelegate() {
      PsiElement elem = getDelegate().resolve();
      return elem instanceof LogtalkClass? (LogtalkClass) elem : null;
    }
    ...
    public PsiMethod findMethodBySignature(PsiMethod patternMethod, boolean checkBases) {
      ...
      //      actual class implementation (getDelegate().resolveLogtalkClass())
      //      and call that.
    ...
    @Override
    public PsiClass findInnerClassByName(String name, boolean checkBases) {
      return null; // No named inner classes in Logtalk.
    }
    ...
    public PsiElement getScope() {
      ...
      while (parent != null) {
        ...
        if (LogtalkTokenTypes.ANONYMOUS_TYPE.equals(parentType)
        || (LogtalkTokenTypes.CLASS_DECLARATION.equals(parentType))
        || parent instanceof LogtalkFile)  // LogtalkFile is a catchall..
        {
          break;
        }
    ...
    @Override
    public PsiIdentifier getNameIdentifier() {
      // For a LogtalkType, the identifier is two children below.  The first is
      // a reference.
      LogtalkReferenceExpression ref = PsiTreeUtil.getRequiredChildOfType(this, LogtalkReferenceExpression.class);
      if (ref.getFirstChild() instanceof LogtalkReferenceExpression) {
        ref = UsefulPsiTreeUtil.getLastChild(ref, LogtalkReferenceExpression.class, ref);
      }
      LogtalkIdentifier id = PsiTreeUtil.getRequiredChildOfType(ref, LogtalkIdentifier.class);
      return id;
    }

    @Override
    public boolean isInheritorDeep(PsiClass baseClass, PsiClass classToByPass) {
      LogtalkClass resolved = getResolvedDelegate();
    ...
    @NotNull
    public PsiReferenceList getExtendsList() {
      // Logtalk BNF doesn't allow for extends in this position.
      return new LogtalkExtendsDeclarationImpl(new LogtalkDummyASTNode("Empty Extends List"));
    }

    @Override
    public PsiReferenceList getImplementsList() {
      // Logtalk BNF doesn't allow for implements in this position.
      return new LogtalkImplementsDeclarationImpl(new LogtalkDummyASTNode("Empty Implements List"));
    }
