 ...
 */
package com.intellij.plugins.logtalk.lang.util;
...
import com.intellij.plugins.haxe.lang.parser.LogtalkAstFactory;
...
import static com.intellij.plugins.haxe.lang.lexer.LogtalkTokenTypes.*;
import static com.intellij.plugins.haxe.lang.util.LogtalkAstUtil.*;
 ...
 */
public class LogtalkConditionalExpression {
  ...
  public LogtalkConditionalExpression(@Nullable ArrayList<ASTNode> startTokens) {
    if (startTokens != null) {
  ...
  private void createToken(@NotNull CharSequence chars, @NotNull IElementType tokenType) {
    tokens.add(LogtalkAstFactory.leaf(tokenType, chars));
    evaluated = false;
  }
   ...
   * LogtalkConditionalExpressionEvaluator class.
   * =================================================================================================
   */
  ...
  public static Key<Object> DEFINES_KEY = Key.create("logtalk.test.defines");
   ...
   *         and: (( cpp || js ) && (logtalk-ver < 3))  -> cpp js || logtalk-ver 3 < &&
  ...
  private Stack<ASTNode> infixToRPN() throws CalculationException {
      ...
      for (ASTNode token : tokens) {
        ...
        if (isLiteral(token) || isStringQuote(token) || isString(token)) {
          ...
          while (!operatorStack.isEmpty()
                 ...
                 && LogtalkOperatorPrecedenceTable.shuntingYardCompare(token.getElementType(), operatorStack.peek().getElementType())) {
    ...
    } catch (LogtalkOperatorPrecedenceTable.OperatorNotFoundException e) {
      LOG.warn("IntelliJ-Logtalk plugin internal error: Unknown operator encountered while calculating compiler conditional exression:"
               + toString(), e);
  ...
  @NotNull
  private LogtalkOperatorPrecedenceTable.Arity getArity(@NotNull ASTNode node)
    throws CalculationException {
    LogtalkOperatorPrecedenceTable.Arity arity = LogtalkOperatorPrecedenceTable.getArity(node.getElementType());
    if (null == arity) {
  ...
  private Object lookupIdentifier(ASTNode identifier) throws CalculationException {
    ...
    if (ApplicationManager.getApplication().isUnitTestMode()) {
      ...
      definitions = LogtalkProjectSettings.getInstance(context).getUserCompilerDefinitions();
    }
  ...
  // Parodies Logtalk parser is_true function
  // https://github.com/LogtalkFoundation/haxe/blob/development/src/syntax/parser.mly#L1596
  ...
  // Parodies Logtalk parser cmp function
  // https://github.com/LogtalkFoundation/haxe/blob/development/src/syntax/parser.mly#L1600
  ...
  // Parodies Logtalk parser eval function
  // https://github.com/LogtalkFoundation/haxe/blob/development/src/syntax/parser.mly#L1619
  @NotNull
  ...
  // Parodies Logtalk parser eval function at lines 1617, 1618, and 1621-1634
  // https://github.com/LogtalkFoundation/haxe/blob/development/src/syntax/parser.mly#L1617
  @NotNull
