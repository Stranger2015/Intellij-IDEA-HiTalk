...
/**
 * Manage a list of Logtalk libraries.  The same library will NOT appear twice in a single
 * list.
 */
public class LogtalkLibraryList {

  static Logger LOG = Logger.getInstance("#LogtalkLibraryList");
  ...
  private Set<LogtalkLibraryReference> myOrderedEntries;
   ...
   */
  LogtalkLibraryList(LogtalkLibraryList initialList) {
    this(initialList.mySdk);
    synchronized(initialList) {
      if (initialList.hasEntries()) {
        Set<LogtalkLibraryReference> entries = getEntries(initialList.size());
        for (LogtalkLibraryReference ref : initialList.getEntries()) {
  ...
  LogtalkLibraryList(@NotNull Sdk sdk, Collection<? extends LogtalkLibraryReference> references) {
    this(sdk);
    getEntries(references.size()).addAll(references);
  }

  LogtalkLibraryList(@NotNull Module module) {
    this(LogtalklibSdkUtils.lookupSdk(module));
  }

  LogtalkLibraryList(@NotNull Sdk sdk) {
    this(sdk, MINIMUM_SIZE);
  }

  LogtalkLibraryList(@NotNull Sdk sdk, int sizeHint) {
   ...
   */
  protected Set<LogtalkLibraryReference> getEntries() {
   ...
   * @return the order entries for this object.
   */
  protected Set<LogtalkLibraryReference> getEntries(int sizeHint) {
    synchronized (this) {
      if (null == myOrderedEntries) {
        ...
        myOrderedEntries = new LinkedHashSet<LogtalkLibraryReference>(2 * sizeHint);
      }
   ...
   */
  public void add(@Nullable LogtalkLibraryReference item) {
    if (null == item)
      return;
   ...
   */
  public void addAll(@Nullable Collection<LogtalkLibraryReference> entries) {
   ...
   */
  public void addAll(LogtalkLibraryList libraries) {
   ...
   * @return
   */
  public boolean contains(@Nullable LogtalkLibraryReference item) {
    if (null == item)
      return false;
   ...
   * @return a LogtalkClasspath containing directories that correspond to the libraries in this list.
   */
  @NotNull
  public LogtalkClasspath getLibraryClasspaths(boolean includeLibRoot, boolean includeSrcRoot) {
    if (!hasEntries()) {
      return LogtalkClasspath.EMPTY_CLASSPATH;
    }
    synchronized(this) {
      Set<LogtalkLibraryReference> entries = getEntries();
      LogtalkClasspath classpath = new LogtalkClasspath(entries.size());
      for (LogtalkLibraryReference ref : entries) {
        LogtalkLibrary lib = ref.getLibrary();
  ...
  public boolean iterate(@NotNull Lambda lambda) {
    ...
    if (hasEntries()) {
      synchronized (this) {
        for (LogtalkLibraryReference entry : getEntries()) {
   ...
   */
  public void remove(@Nullable LogtalkLibraryReference item) {
   ...
   */
  public void removeAll(@Nullable LogtalkLibraryList otherPath) {
   ...
   */
  public void removeAll(@Nullable Collection<LogtalkLibraryReference> otherEntries) {
