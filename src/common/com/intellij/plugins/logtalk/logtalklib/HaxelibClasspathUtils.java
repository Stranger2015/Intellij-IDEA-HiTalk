 ...
 */
public class LogtalklibClasspathUtils {

  static Logger LOG = Logger.getInstance("#LogtalklibClasspathUtils");
  static {
    LOG.setLevel(Level.DEBUG);
  }

  /** Old environment variable name for the Logtalk standard library location. */
  final public static String HAXE_LIBRARY_PATH = "HAXE_LIBRARY_PATH";
  /** Modern environment variable name for the Logtalk standard library location. */
   ...
   */
  @NotNull
  public static LogtalkClasspath getUnmanagedProjectLibraryClasspath(@NotNull Project project) {
    LibraryTable libraryTable = ProjectLibraryTable.getInstance(project);
    if (null == libraryTable) return LogtalkClasspath.EMPTY_CLASSPATH;

    LogtalkClasspath classpath = new LogtalkClasspath();
    ...
    for (Library library : libraries) {
      ...
      //
      if (!LogtalklibNameUtil.isManagedLibrary(library.getName())) {
        ...
        for (OrderRootType rootType : interestingRootTypes) {
          for (String url : library.getUrls(rootType)) {
            if (!classpath.containsUrl(url)) {  // The if just keeps us from churning.
              classpath.add(new LogtalkClasspathEntry(library.getName(), url));
            }
          }
        }
      }
  ...
  public static List<String> getProjectLibraryNames(@NotNull Project project, boolean filterManagedLibs) {
    ...
    Library[] libraries = libraryTable.getLibraries();
    for (Library library : libraries) {
      if (filterManagedLibs && LogtalklibNameUtil.isManagedLibrary(library.getName())) {
        continue;
      }
   ...
   */
  @NotNull
  private static LogtalkClasspath loadClasspathFrom(LibraryTable libraryTable) {
    LogtalkClasspath classpath = new LogtalkClasspath();
    Library[] libraries = libraryTable.getLibraries();
    OrderRootType interestingRootTypes[] = {OrderRootType.SOURCES, OrderRootType.CLASSES};
    for (Library library : libraries) {
   ...
   */
  @NotNull
  public static LogtalkClasspath getProjectLibraryClasspath(@NotNull Project project) {
    LibraryTable libraryTable = ProjectLibraryTable.getInstance(project);
    if (null == libraryTable) return LogtalkClasspath.EMPTY_CLASSPATH;
   ...
   */
  @NotNull
  public static LogtalkClasspath getSdkClasspath(@NotNull Sdk sdk) {
    LogtalkClasspath classpath = new LogtalkClasspath();
    ...
    for (OrderRootType rootType : interestingRootTypes) {
      for (VirtualFile file : rootProvider.getFiles(rootType)) {
        if (!classpath.containsUrl(file.getUrl())) {
          classpath.add(new LogtalklibItem(file.getName(), file.getUrl()));
        }
      }
    }
    return classpath;
   ...
   */
  @NotNull
  public static LogtalkClasspath getModuleClasspath(@NotNull Module module) {
    ModuleRootManager rootManager = ModuleRootManager.getInstance(module);
    if (null == rootManager) return LogtalkClasspath.EMPTY_CLASSPATH;

    ModifiableRootModel rootModel = rootManager.getModifiableModel();
    LibraryTable libraryTable = rootModel.getModuleLibraryTable();
    LogtalkClasspath moduleClasspath = loadClasspathFrom(libraryTable);
   ...
   * @return LogtalkClasspath according to the module compilation options.
   */
  @NotNull
  public static LogtalkClasspath getImplicitClassPath(@NotNull Module module) {
    ...
    LogtalkTarget target = settings.getCompilationTarget();
    ...
    if (null != targetFlag) {
      cp.add(new LogtalkClasspathEntry("Std library target implementation",
                                    libraryPath + File.separator + targetFlag +
                                      File.separator + "_std"));
    }
    cp.add(new LogtalkClasspathEntry("Standard Library", libraryPath));
    return cp;
  }
   ...
   */
  @NotNull
  public static LogtalkClasspath getFullClasspath(@NotNull Module module) {
    LogtalkClasspath classpath = new LogtalkClasspath(getImplicitClassPath(module));
    ...
    classpath.addAll(getSdkClasspath(LogtalklibSdkUtils.lookupSdk(module)));
    return classpath;
  }
   ...
   */
  @NotNull
  public static List<String> getLogtalklibLibraryPathUrl(@NotNull Sdk sdk, @NotNull String name) {
    List<String> strings = LogtalklibCommandUtils.issueLogtalklibCommand(sdk, "path",
                                                                   name);
   ...
   *
   * @param name name of the base file or library to search for.
   * @return a set of LogtalklibItems, may be an empty list.
   */
  @NotNull
  public static LogtalkClasspath getLogtalklibLibraryPath(@NotNull Sdk sdk, @NotNull String name) {
    List<String> strings = LogtalklibCommandUtils.issueLogtalklibCommand(sdk, "path", name);
    LogtalkClasspath classpath = new LogtalkClasspath(strings.size());
    ...
    for (String string : strings) {
      if (!string.startsWith("-L") && !string.startsWith("-D")) {
        ...
        if (file != null) {
          ...
          classpath.add(new LogtalklibItem(file.getPath(), file.getUrl()));
        }
      }
    }
  ...
  public static List<String> getInstalledLibraries(@NotNull Sdk sdk) {
    ...
    List<String> installedLogtalklibs = new ArrayList<String>();
    for (String s : LogtalklibCommandUtils.issueLogtalklibCommand(sdk, "list")) {
      installedLogtalklibs.add(s.split(":")[0]);
    }
  ...
  /**
   * Find logtalk libraries matching a search word, using the version of haxelib
  ...
  public static List<String> getAvailableLibrariesMatching(@NotNull Sdk sdk, @NotNull String word) {
    List<String> stringList = LogtalklibCommandUtils.issueLogtalklibCommand(sdk, "search", word);
  ...
  public static List<String> getProjectDisplayInformation(@NotNull Project project, @NotNull File dir, @NotNull String executable, @NotNull Sdk sdk) {
    ...
    if (getInstalledLibraries(sdk).contains(executable)) {
      ...
      commandLineArguments.add(LogtalklibCommandUtils.getLogtalklibPath(sdk));
      ...
      List<String> strings = LogtalklibCommandUtils.getProcessStdout(commandLineArguments,
                                                                  dir,
                                                                  LogtalkSdkUtilBase.getSdkData(sdk));
      String s = Joiner.on("\n").join(strings);
  ...
  public static VirtualFile findFileOnClasspath(@NotNull final Module module, @NotNull final String filePath) {
    ...
    return ApplicationManager.getApplication().runReadAction(new Computable<VirtualFile>() {
      ...
      public VirtualFile compute() {
        ...
        if (null == found) {
          // This grabs either the module's SDK, or the inherited one, if any.
          found = findFileOnOneClasspath(getSdkClasspath(LogtalklibSdkUtils.lookupSdk(module)), filePath);
        }
        return found;
      }
    });
   ...
   */
  @Nullable
  private static VirtualFile findFileOnOneClasspath(@NotNull LogtalkClasspath cp, @NotNull final String filePath) {
    final VirtualFileManager vfmInstance = VirtualFileManager.getInstance();

    class MyLambda implements LogtalkClasspath.Lambda {
  ...
  public static VirtualFile findFirstFileOnClasspath(@NotNull final Module module,
    ...
    final VirtualFileManager vfmInstance = VirtualFileManager.getInstance();
    final LogtalkClasspath cp = ApplicationManager.getApplication().runReadAction(new Computable<LogtalkClasspath>() {
      @Override
      public LogtalkClasspath compute() {
