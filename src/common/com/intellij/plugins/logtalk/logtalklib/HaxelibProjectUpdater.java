...
import com.intellij.plugins.haxe.util.LogtalkDebugUtil;
...
/**
 * Manages Logtalk library class paths across projects.
 *
 ...
 * various types of Logtalk project definitions (OpenFL, NME, etc.) and adding
 ...
 *
 */
public class LogtalklibProjectUpdater {

  static Logger LOG = Logger.getInstance("#com.intellij.plugins.logtalk.haxelib.LogtalklibManager");
  ...
  public static final LogtalklibProjectUpdater INSTANCE = new LogtalklibProjectUpdater();

  private ProjectUpdateQueue myQueue = null;
  private ProjectMap myProjects = null;

  private LogtalklibProjectUpdater() {
    myQueue = new ProjectUpdateQueue();
    myProjects = new ProjectMap();
  }

  @NotNull
  static LogtalklibProjectUpdater getInstance() {
    return INSTANCE;
  }
  ...
  /**
   * Retrieve the LogtalklibLibraryCacheManager for a given module/project.
   * <p>
   ...
   *
   * @param module that we need the LogtalklibLibraryCacheManager for.
   * @return the appropriate LogtalklibLibraryCacheManager.
   */
  @Nullable
  public LogtalklibLibraryCacheManager getLibraryCacheManager(@NotNull Module module) {
   ...
   * unless we move the notion of a project into it.
   *
   * @param project that we need the LogtalklibLibraryCacheManager for.
   * @return the appropriate LogtalklibLibraryCacheManager.
   */
  @Nullable
  public LogtalklibLibraryCacheManager getLibraryCacheManager(@NotNull Project project) {
    ProjectTracker tracker = myProjects.get(project);
   ...
   *
   * @param module
   * @return the LogtalklibLibraryCache for the module, if found; null, otherwise.
   */
  @Nullable
  public static LogtalklibLibraryCache getLibraryCache(@NotNull Module module) {
    LogtalklibLibraryCacheManager mgr = LogtalklibProjectUpdater.getInstance().getLibraryCacheManager(module);
   ...
   *
   * @param project
   * @return the LogtalklibLibraryCache for the project, if found; null, otherwise.
   */
  @Nullable
  public static LogtalklibLibraryCache getLibraryCache(@NotNull Project project) {
    LogtalklibLibraryCacheManager mgr = LogtalklibProjectUpdater.getInstance().getLibraryCacheManager(project);
    Sdk projectSdk = LogtalklibSdkUtils.lookupSdk(project);
   ...
   *                     typically specified in the Logtalk project files. (e.g. -lib)
   */
  private void resolveModuleLibraries(ProjectTracker tracker, Module module, LogtalkLibraryList externalLibs) {
    LogtalkLibraryList toAdd;
    LogtalkLibraryList toRemove;

    toAdd = new LogtalkLibraryList(module);
    toRemove = new LogtalkLibraryList(module);
    ...
    syncLibraryLists(moduleSdk,
                     LogtalklibUtil.getModuleLibraries(module),
                     externalLibs,
        /*modifies*/ toAdd,
        /*modifies*/ toRemove);
  ...
  /**
   * Find an IDEA library matching a LogtalkLibraryReference.
   *
   ...
   */
  @Nullable
  private Library lookupLibrary(@NotNull Iterator<Library> iter, @NotNull LogtalkLibraryReference ref) {
    Library found = null;
  ...
  /**
   * Find an IDEA library in a project's LibraryTable matching a LogtalkLibraryReference.
   *
   ...
   */
  @Nullable
  private Library lookupProjectLibrary(@NotNull LibraryTable table, @NotNull LogtalkLibraryReference ref) {
   ...
   * matching a LogtalkLibraryReference.
   *
   * @param table - the LibraryTable to look in.
   * @param ref - the library to find.
   * @return the Library, if found; null, otherwise.
   */
  @Nullable
  private Library lookupModelLibrary(@NotNull LibraryTable.ModifiableModel table, @NotNull LogtalkLibraryReference ref) {
    return lookupLibrary(table.getLibraryIterator(), ref);
  }
   ...
   */
  private void removeLibraries(@NotNull final LogtalkLibraryList toRemove,
                               @NotNull final LibraryTable.ModifiableModel libraryTableModel,
                               @NotNull final LogtalkDebugTimeLog timeLog) {
    timeLog.stamp("Removing libraries.");
    toRemove.iterate(new LogtalkLibraryList.Lambda() {
      @Override
   ...
   */
  private void addLibraries(@NotNull final LogtalkLibraryList toAdd,
                            @NotNull final LogtalkLibraryList projectLibraries,
    ...
    toAdd.iterate(new LogtalkLibraryList.Lambda() {
      @Override
      public boolean processEntry(LogtalkLibraryReference entry) {
        ...
        if (projectLibraries.contains(entry)) {
          ...
          if (moduleLibrary == null) {
            ...
            try {
              ...
              LogtalkLibrary entryLibrary = entry.getLibrary();
              LogtalkClasspath classpath = entryLibrary != null ? entryLibrary.getClasspathEntries() : null;
              if (null != classpath) {
                classpath.iterate(new LogtalkClasspath.Lambda() {
                  @Override
                  public boolean processEntry(LogtalkClasspathEntry cp) {
                    String url = LogtalkFileUtil.fixUrl(cp.getUrl());
   ...
   */
  private void assertEntriesAreManaged(LogtalkLibraryList list, final String msg) {
    if (null != list) {
      list.iterate(new LogtalkLibraryList.Lambda() {
        @Override
        public boolean processEntry(LogtalkLibraryReference entry) {
  ...
  private void updateModule(final ProjectTracker tracker,
                            final Module module,
                            final LogtalkLibraryList toRemove,
                            final LogtalkLibraryList toAdd) {
    ...
    final LogtalkDebugTimeLog timeLog = new LogtalkDebugTimeLog("Write action:");
    ...
    doWriteAction(new Runnable() {
      ...
      public void run() {
        ...
        LogtalkLibraryList projectLibraries = ModuleRootManager.getInstance(module).isSdkInherited()
                                         ? getProjectLibraryList(tracker)
                                         : new LogtalkLibraryList(module);
   ...
   */
  private void syncOneModule(@NotNull final ProjectTracker tracker, @NotNull Module module, @NotNull LogtalkDebugTimeLog timeLog) {

    Project project = tracker.getProject();
    LogtalkLibraryList haxelibExternalItems = new LogtalkLibraryList(module);
    LogtalklibLibraryCache libManager = tracker.getSdkManager().getLibraryManager(module);
    LogtalkModuleSettings settings = LogtalkModuleSettings.getInstance(module);
    ...
    switch (settings.getBuildConfiguration()) {
      case NMML:
        ...
        LogtalkLibrary nme = libManager.getLibrary("nme", LogtalklibSemVer.ANY_VERSION);
        if (null != nme) {
          haxelibExternalItems.add(nme.createReference(LogtalklibSemVer.ANY_VERSION));
        }
        else {
        ...
        if (nmmlPath != null && !nmmlPath.isEmpty()) {
          ...
          if (file != null && file.getFileType().equals(NMMLFileType.INSTANCE)) {
            ...
            if (psiFile != null && psiFile instanceof XmlFile) {
              haxelibExternalItems.addAll(LogtalklibUtil.getLogtalklibsFromXmlFile((XmlFile)psiFile, libManager));
            }
          }
        }
      ...
      case OPENFL:
        ...
        LogtalkLibrary openfl = libManager.getLibrary("openfl", LogtalklibSemVer.ANY_VERSION);
        if (null != openfl) {
          haxelibExternalItems.add(openfl.createReference(LogtalklibSemVer.ANY_VERSION)); // No specific version.
        }
        else {
        ...
        if (openFLXmlPath != null && !openFLXmlPath.isEmpty()) {
          ...
          List<String> projectClasspaths =
            LogtalklibClasspathUtils.getProjectDisplayInformation(project, dir, "openfl",
                                                               LogtalklibSdkUtils.lookupSdk(module));

          for (String classpath : projectClasspaths) {
            VirtualFile file = LocalFileFinder.findFile(classpath);
            if (file != null) {
              LogtalkLibrary lib = libManager.getLibraryByPath(file.getPath());
              if (null != lib) {
        ...
        if (hxmlPath != null && !hxmlPath.isEmpty()) {
          ...
          if (file != null && file.getFileType().equals(HXMLFileType.INSTANCE)) {
            ...
            if (psiFile != null) {
              ...
              if (libs != null) {
                for (String lib : libs) {
                  LogtalkLibraryReference reference = LogtalkLibraryReference.create(module, lib);
      ...
      default:
        throw new LogtalkDebugUtil.InvalidCaseException(settings.getBuildConfiguration());
    }
  ...
  private void syncModuleClasspaths(final ProjectTracker tracker) {
    final LogtalkDebugTimeLog timeLog = LogtalkDebugTimeLog.startNew("syncModuleClasspaths");
    ...
    Collection<Module> modules = ModuleUtil.getModulesOfType(project, LogtalkModuleType.getInstance());
    int i = 0;
   ...
   *
   * @param tracker for the project being updated.
   * @return a LogtalkClasspath representing the libraries specified at the project level.
   */
  @NotNull
  private LogtalkLibraryList getProjectLibraryList(@NotNull ProjectTracker tracker) {
    ProjectLibraryCache cache = tracker.getCache();
    LogtalkLibraryList projectLibraries;
    LogtalkConfiguration buildConfig = LogtalkConfiguration.CUSTOM; // Only properties available.

    if (cache.isListSetFor(buildConfig)) {
      projectLibraries = cache.getListFor(buildConfig);
    }
    else {
      projectLibraries = LogtalklibUtil.getProjectLibraries(tracker.getProject(), false, false);
   ...
   */
  @NotNull
  private LogtalkLibraryList collectDependencies(@NotNull LogtalkLibraryList list) {
    final LogtalkLibraryList dependencies = new LogtalkLibraryList(list.getOwner());
    list.iterate(new LogtalkLibraryList.Lambda() {
      @Override
      public boolean processEntry(LogtalkLibraryReference entry) {
   ...
   */
  @NotNull
  private void markAsManaged(@NotNull LogtalkLibraryList list) {
    list.iterate(new LogtalkLibraryList.Lambda() {
      @Override
      public boolean processEntry(LogtalkLibraryReference entry) {
   ...
   */
  @NotNull
  private LogtalkLibraryList findLibsMatchingClasspath(@NotNull final LogtalkLibraryList libs, @NotNull final LogtalkClasspath classpath) {
    final LogtalkLibraryList matchingLibs = new LogtalkLibraryList(libs.getOwner());
    libs.iterate(new LogtalkLibraryList.Lambda() {
      @Override
      public boolean processEntry(LogtalkLibraryReference entry) {
        LogtalkLibrary lib = entry.getLibrary();
  ...
  private void syncLibraryLists(@NotNull Sdk sdk,
                                @NotNull LogtalkLibraryList currentList,
                                @NotNull LogtalkLibraryList externallyRequired,
                   /*modifies*/ @NotNull LogtalkLibraryList newLibrariesToAdd,
                   /*modifies*/ @NotNull LogtalkLibraryList oldLibrariesToRemove) {

    final LogtalkLibraryList currentManagedEntries = new LogtalkLibraryList(sdk);
    final LogtalkLibraryList currentUnmanagedEntries = new LogtalkLibraryList(sdk);
    currentList.iterate(new LogtalkLibraryList.Lambda() {
      @Override
      public boolean processEntry(LogtalkLibraryReference entry) {
    ...
    LogtalkLibraryList dependencies = new LogtalkLibraryList(collectDependencies(externallyRequired));
    ...
    LogtalkLibraryList toRemove = currentManagedEntries;

    // We want to add all externally required entries
    LogtalkLibraryList toAdd = new LogtalkLibraryList(sdk);
  ...
  private void syncProjectClasspath(@NotNull ProjectTracker tracker) {
    Sdk sdk = LogtalklibSdkUtils.lookupSdk(tracker.getProject());
    boolean isLogtalkSDK = sdk.getSdkType().equals(LogtalkSdkType.getInstance());

    if (!isLogtalkSDK) {
      return;
    }

    LogtalkDebugTimeLog timeLog = new LogtalkDebugTimeLog("syncProjectClasspath");
    timeLog.stamp("Start synchronizing project " + tracker.getProject().getName());

    LogtalkLibraryList toAdd = new LogtalkLibraryList(sdk);
    LogtalkLibraryList toRemove = new LogtalkLibraryList(sdk);
    syncLibraryLists(sdk,
                     LogtalklibUtil.getProjectLibraries(tracker.getProject(),false, false),
                     new LogtalkLibraryList(sdk),
        /*modifies*/ toAdd,
        /*modifies*/ toRemove );
    ...
    tracker.getCache().setPropertiesList(LogtalklibUtil.getProjectLibraries(tracker.getProject(), false, false));
  }
  ...
  private void updateProject(@NotNull final ProjectTracker tracker,
                             final @Nullable LogtalkLibraryList toRemove,
                             final @Nullable LogtalkLibraryList toAdd) {
    ...
    doWriteAction(new Runnable() {
      ...
      public void run() {
        ...
        if (null != toAdd) {
          ...
          toAdd.iterate(new LogtalkLibraryList.Lambda() {
            @Override
            public boolean processEntry(LogtalkLibraryReference newItem) {
  ...
  final private class ProjectLibraryCache {
    ...
    private Sdk sdk;
    private LogtalkLibraryList nmmlList;
    private LogtalkLibraryList openFLList;
    private LogtalkLibraryList hxmlList;
    private LogtalkLibraryList propertiesList;
    ...
    public void clear() {
      setNmmlList(new LogtalkLibraryList(sdk));
      setOpenFLList(new LogtalkLibraryList(sdk));
      setHxmlList(new LogtalkLibraryList(sdk));
      setPropertiesList(new LogtalkLibraryList(sdk));

      // Reset the 'set' bits.
      nmmlIsSet = openFLIsSet = hxmlIsSet = propertiesIsSet = false;
    }

    public boolean isListSetFor(LogtalkConfiguration buildConfig) {
      switch(buildConfig) {
        case NMML:
          return nmmlIsSet;
        case OPENFL:
    ...
    @NotNull
    public LogtalkLibraryList getListFor(LogtalkConfiguration buildConfig) {
      switch(buildConfig) {
        case NMML:
          return getNmmlList();
        case OPENFL:
      ...
      return new LogtalkLibraryList(sdk);
    }

    public void setListFor(LogtalkConfiguration buildConfig, LogtalkLibraryList list) {
      switch(buildConfig) {
        case NMML:
          setNmmlList(list);
        case OPENFL:
    ...
    @NotNull
    public LogtalkLibraryList getNmmlList() {
  return nmmlList != null ? nmmlList : new LogtalkLibraryList(sdk);
    }

    @NotNull
    public LogtalkLibraryList getOpenFLList() {
      return openFLList != null ? openFLList : new LogtalkLibraryList(sdk);
    }

    @NotNull
    public LogtalkLibraryList getHxmlList() {
      return hxmlList != null ? hxmlList : new LogtalkLibraryList(sdk);
    }

    @NotNull
    public LogtalkLibraryList getPropertiesList() {
      return propertiesList != null ? propertiesList : new LogtalkLibraryList(sdk);
    }

    public void setNmmlList(LogtalkLibraryList nmmlList)  {
      this.nmmlList = nmmlList;
      nmmlIsSet = true;
    }

    public void setOpenFLList(LogtalkLibraryList openFLList) {
      this.openFLList = openFLList;
      openFLIsSet = true;
    }

    public void setHxmlList(LogtalkLibraryList hxmlList) {
      this.hxmlList = hxmlList;
      hxmlIsSet = true;
    }

    public void setPropertiesList(LogtalkLibraryList propertiesList) {
  ...
  public final class ProjectTracker {
    ...
    LogtalklibLibraryCacheManager mySdkManager;
    ...
    public ProjectTracker(Project project) {
      ...
      myIsUpdating = false;
      myReferenceCount = 0;
      myCache = new ProjectLibraryCache(LogtalklibSdkUtils.lookupSdk(project));
      mySdkManager = new LogtalklibLibraryCacheManager();
    }
     ...
     */
    @NotNull
    public LogtalklibLibraryCacheManager getSdkManager() {
      return mySdkManager;
    }
