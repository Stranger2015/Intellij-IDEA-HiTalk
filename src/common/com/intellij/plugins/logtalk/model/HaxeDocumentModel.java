...
import com.intellij.plugins.haxe.util.LogtalkCharUtils;
import com.intellij.plugins.logtalk.util.LogtalkCharUtils;
import com.intellij.psi.PsiDocumentManager;
import com.intellij.psi.PsiElement;

public class LogtalkDocumentModel {
  private Document document;

  public LogtalkDocumentModel(Document document) {
    this.document = document;
  }

  public LogtalkDocumentModel(PsiElement aElementInDocument) {
    this(PsiDocumentManager.getInstance(aElementInDocument.getProject()).getDocument(aElementInDocument.getContainingFile()));
  }

  static public LogtalkDocumentModel fromElement(PsiElement aElementInDocument) {
    return new LogtalkDocumentModel(aElementInDocument);
  }
  ...
  public void replaceElementText(final TextRange range, final String text, final StripSpaces strips) {
    ...
    if (strips.after) {
      while (end < documentText.length() && LogtalkCharUtils.isSpace(documentText.charAt(end))) {
        end++;
      }
    }
    if (strips.before) {
      while (start > 0 && LogtalkCharUtils.isSpace(documentText.charAt(start - 1))) {
        start--;
      }
    }
