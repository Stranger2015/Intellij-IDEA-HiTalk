...
import com.intellij.plugins.haxe.lang.psi.LogtalkTypeOrAnonymous;
...
public class LogtalkTypeCompatible {
  ...
  static public boolean canAssignToFrom(
    ...
    if (to instanceof SpecificLogtalkClassReference && from instanceof SpecificLogtalkClassReference) {
      return canAssignToFromType((SpecificLogtalkClassReference)to, (SpecificLogtalkClassReference)from);
    }
  ...
  static private boolean canAssignToFromType(
    @NotNull SpecificLogtalkClassReference to,
    @NotNull SpecificLogtalkClassReference from
  ) {
    ...
    LogtalkClass thisClassPsi = to.clazz.getLogtalkClass();
    if (thisClassPsi != null) {
      LogtalkClassModel thisClass = thisClassPsi.getModel();
      for (LogtalkType type : thisClass.getAbstractFromList()) {
        if (LogtalkTypeResolver.getTypeFromType(type).toStringWithoutConstant().equals(from.toStringWithoutConstant())) {
          return true;
        }
      }
    }
    ...
    // Check to abstracts
    LogtalkClass thatClassPsi = from.clazz.getLogtalkClass();
    if (thatClassPsi != null) {
      LogtalkClassModel thatClass = thatClassPsi.getModel();

      if (thatClass.isAbstract()) {

        // Check if this is required!
        LogtalkTypeOrAnonymous underlyingAbstractType = thatClass.getUnderlyingType();
        if (underlyingAbstractType != null) {
          ResultHolder thatUnderlying = LogtalkTypeResolver.getTypeFromTypeOrAnonymous(underlyingAbstractType);
          if (to.toStringWithoutConstant().equals(thatUnderlying.toStringWithoutConstant())) {
            return true;
          }
        }

        for (LogtalkType type : thatClass.getAbstractToList()) {
          if (to.canAssign(LogtalkTypeResolver.getTypeFromType(type))) {
            return true;
          }
        }
      }
    ...
    LogtalkClass baseClass = to.clazz.getLogtalkClass();
    LogtalkClass derivedClass = from.clazz.getLogtalkClass();
    if (baseClass != null && derivedClass != null) {
      final HashSet<LogtalkClass> set = LogtalkResolveUtil.getBaseClassesSet(derivedClass, new HashSet<LogtalkClass>());
