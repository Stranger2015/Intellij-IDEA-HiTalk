...
public class LogtalkTypeUnifier {
  @NotNull
  ...
  static public SpecificTypeReference unify(SpecificTypeReference a, SpecificTypeReference b, @NotNull PsiElement context) {
    ...
    if (a instanceof SpecificLogtalkClassReference && b instanceof SpecificLogtalkClassReference) {
      return unifyTypes((SpecificLogtalkClassReference)a, (SpecificLogtalkClassReference)b, context);
    }
  ...
  static public SpecificTypeReference unifyFunctions(SpecificFunctionReference a, SpecificFunctionReference b, @NotNull PsiElement context) {
    ...
    //if (pa.size() != pb.size()) throw new LogtalkCannotUnifyException();
  ...
  @NotNull
  static public SpecificTypeReference unifyTypes(SpecificLogtalkClassReference a, SpecificLogtalkClassReference b, @NotNull PsiElement context) {
    if (a.isDynamic()) return a.withoutConstantValue();
    if (b.isDynamic()) return b.withoutConstantValue();
    if (a.getLogtalkClassModel() == null) return SpecificTypeReference.getDynamic(context);
    if (b.getLogtalkClassModel() == null) return SpecificTypeReference.getDynamic(context);
    final Set<LogtalkClassModel> atypes = a.getLogtalkClassModel().getCompatibleTypes();
    final Set<LogtalkClassModel> btypes = b.getLogtalkClassModel().getCompatibleTypes();
    // @TODO: this could be really slow, hotspot for optimizing
    for (LogtalkClassModel type : atypes) {
      if (btypes.contains(type)) {
        // @TODO: generics
        return SpecificLogtalkClassReference.withoutGenerics(
          new LogtalkClassReference(type, context)
        );
      }
    }
