...
import com.intellij.plugins.logtalk.model.LogtalkParameterModel;
...
public class SpecificFunctionReference extends SpecificTypeReference {
  ...
  @Nullable
  final public LogtalkMethodModel method;

  public SpecificFunctionReference(List<ResultHolder> params, ResultHolder retval, @Nullable LogtalkMethodModel method, @NotNull PsiElement context) {
    super(context);
    this.params = params;
    this.retval = retval;
    this.method = method;
  ...
  public int getNonOptionalArgumentsCount() {
    if (method != null) {
      int count = 0;
      for (LogtalkParameterModel param : method.getParameters()) {
        if (!param.isOptional()) {
          count++;
        } else {
          break;
