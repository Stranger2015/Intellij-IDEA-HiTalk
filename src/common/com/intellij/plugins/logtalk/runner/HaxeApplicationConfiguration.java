...
import com.intellij.plugins.haxe.runner.ui.LogtalkRunConfigurationEditorForm;
import com.intellij.plugins.logtalk.LogtalkBundle;
import com.intellij.plugins.logtalk.runner.ui.LogtalkRunConfigurationEditorForm;
...
public class LogtalkApplicationConfiguration extends ModuleBasedConfiguration<LogtalkApplicationModuleBasedConfiguration>
  ...
  public LogtalkApplicationConfiguration(String name, Project project, LogtalkRunConfigurationType configurationType) {
    super(name, new LogtalkApplicationModuleBasedConfiguration(project), configurationType.getConfigurationFactories()[0]);
  }
  ...
  protected ModuleBasedConfiguration createInstance() {
    return new LogtalkApplicationConfiguration(getName(), getProject(), LogtalkRunConfigurationType.getInstance());
  }

  public SettingsEditor<? extends RunConfiguration> getConfigurationEditor() {
    return new LogtalkRunConfigurationEditorForm(getProject());
  }
  ...
  public void checkConfiguration() throws RuntimeConfigurationException {
    ...
    final LogtalkApplicationModuleBasedConfiguration configurationModule = getConfigurationModule();
    final Module module = configurationModule.getModule();
    if (module == null) {
      throw new RuntimeConfigurationException(LogtalkBundle.message("logtalk.run.no.module", getName()));
    }
    ...
    if (settings.isUseHxmlToBuild() && !customFileToLaunch) {
      throw new RuntimeConfigurationException(LogtalkBundle.message("logtalk.run.select.custom.file"));
    }
    if (settings.isUseNmmlToBuild() && customFileToLaunch) {
      throw new RuntimeConfigurationException(LogtalkBundle.message("logtalk.run.do.not.select.custom.file"));
    }
    if (settings.isUseNmmlToBuild() && customExecutable) {
      throw new RuntimeConfigurationException(LogtalkBundle.message("logtalk.run.do.not.select.custom.executable"));
    }
  }
