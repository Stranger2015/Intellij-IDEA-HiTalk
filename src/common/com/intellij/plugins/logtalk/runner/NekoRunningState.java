...
import com.intellij.plugins.logtalk.config.sdk.LogtalkSdkData;
...
public class NekoRunningState extends CommandLineState {
  ...
  protected ProcessHandler startProcess() throws ExecutionException {
    ...
    assert sdk != null;
    assert settings.getLogtalkTarget() == LogtalkTarget.NEKO;
    final LogtalkSdkData sdkData = sdk.getSdkAdditionalData() instanceof LogtalkSdkData ? (LogtalkSdkData)sdk.getSdkAdditionalData() : null;

    LogtalkCommandLine commandLine = getCommandForNeko(sdkData, settings);

    return new ColoredProcessHandler(commandLine.createProcess(), commandLine.getCommandLineString());
  }

  private LogtalkCommandLine getCommandForNeko(@Nullable LogtalkSdkData sdkData, LogtalkModuleSettings settings) throws ExecutionException {
    if (sdkData == null || sdkData.getNekoBinPath() == null || sdkData.getNekoBinPath().isEmpty()) {
      throw new ExecutionException(LogtalkBundle.message("logtalk.run.bad.neko.bin.path"));
    }

    LogtalkCommandLine commandLine = new LogtalkCommandLine(module);
