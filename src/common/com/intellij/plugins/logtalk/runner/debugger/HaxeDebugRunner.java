...
import com.intellij.plugins.logtalk.hxml.model.HXMLProjectModel;
import com.intellij.plugins.logtalk.logtalklib.LogtalklibClasspathUtils;
import com.intellij.plugins.haxe.ide.module.LogtalkModuleSettings;
import com.intellij.plugins.haxe.runner.LogtalkApplicationConfiguration;
...
import com.intellij.plugins.logtalk.runner.LogtalkApplicationConfiguration;
import com.intellij.plugins.logtalk.runner.NMERunningState;
import com.intellij.plugins.logtalk.runner.OpenFLRunningState;
...
import logtalk.root.JavaProtocol;
 ...
 * <p/>
 * This is the singular Logtalk debug runner, that can debug:
 * 1. Flash targets
 * 2. Hxcpp targets, run locally by the IDE
 * 3. Hxcpp targets, run by an external command
 */
public class LogtalkDebugRunner extends DefaultProgramRunner {
  public static final String HAXE_DEBUG_RUNNER_ID = "LogtalkDebugRunner";
  ...
  public boolean canRun(@NotNull String executorId,
    ...
    return (DefaultDebugExecutor.EXECUTOR_ID.equals(executorId) &&
            (profile instanceof LogtalkApplicationConfiguration));
  }

  @Override
  protected RunContentDescriptor doExecute(RunProfileState state, ExecutionEnvironment env)
    throws ExecutionException {
    final LogtalkApplicationConfiguration configuration =
      (LogtalkApplicationConfiguration)(env.getRunProfile());
    final Module module =
    ...
    if (module == null) {
      throw new ExecutionException
        (LogtalkBundle.message("no.module.for.run.configuration",
                               configuration.getName()));
    }

    final LogtalkModuleSettings settings =
      LogtalkModuleSettings.getInstance(module);
    ...
    if (settings.isUseHxmlToBuild() || settings.isUseUserPropertiesToBuild()) {
      ...
      else if (settings.getLogtalkTarget() == LogtalkTarget.CPP) {
        hxcppDebug = true;
      }
    }
    ...
    if (flashDebug) {
      ...
      if(configuration.isCustomFileToLaunch()) {
        ...
        if (!settings.isUseOpenFLToBuild()) {
          flashFileToDebug = LogtalkCompilerUtil.calculateCompilerOutput(module);
        } else {
          HXMLProjectModel lime = LimeUtil.getLimeProjectModel(module, true);
          flashFileToDebug = LogtalkFileUtil.joinPath(module.getProject().getBasePath(), lime.getSwfOutputFileName());
        }
      }
      ...
      throw new ExecutionException
        (LogtalkBundle.message("logtalk.proper.debug.targets"));
    }
  }
  ...
  private RunContentDescriptor runFlash(final Module module,
    ...
    if (plugin == null) {
      throw new ExecutionException
        (LogtalkBundle.message("install.flex.plugin"));
    }
    if (!plugin.isEnabled()) {
      throw new ExecutionException
        (LogtalkBundle.message("enable.flex.plugin"));
    }

    String flexSdkName = settings.getFlexSdkName();
    if (StringUtil.isEmpty(flexSdkName)) {
      throw new ExecutionException
        (LogtalkBundle.message("flex.sdk.not.specified"));
    }

    if (settings.isUseNmmlToBuild()) {
      return LogtalkFlashDebuggingUtil.getNMEDescriptor
        (this, module, env, executor, flexSdkName);
    }
    else if (settings.isUseOpenFLToBuild()) {
      return LogtalkFlashDebuggingUtil.getOpenFLDescriptor
        (this, module, env, executor, flexSdkName);
    }
    else {
      return LogtalkFlashDebuggingUtil.getDescriptor
                   ...
                   debugProcess.setExecutionResult
                     (new OpenFLRunningState
                        (env, module,
                         ...
                         true, port).
                       execute(executor, LogtalkDebugRunner.this));
                 }
                 else {
                   debugProcess.setExecutionResult
                     (new NMERunningState
  ...
  private class DebugProcess extends XDebugProcess {
    ...
    public XDebuggerEditorsProvider getEditorsProvider() {
      return new LogtalkDebuggerEditorsProvider();
    }
    ...
    private void info(String message) {
      showInfoMessage(mProject, message, "Logtalk Debugger");
    }

    private void warn(String message) {
      showInfoMessage(mProject, message, "Logtalk Debugger Warning");
    }

    private void error(String message) {
      showInfoMessage(mProject, message, "Logtalk Debugger Error");
      this.stop();
    }
...
//                         JavaProtocol.messageToString(message));
    ...
    private class StackFrame extends XStackFrame {
      public StackFrame(final Project project, final Module module,
        ...
        if (null == file || !file.exists()) {
          ...
          if (matches.isEmpty()) {
            ...
            // an appropriate file.
            file = LogtalklibClasspathUtils.findFileOnClasspath(module, mFileName);
          }
            ...
            file = possible.isValid() ? possible : LogtalklibClasspathUtils.findFileOnClasspath(module, possible.toString());
          }
          else {
            // Too many matches. Get the first that occurs on the classpath.
            file = LogtalklibClasspathUtils.findFirstFileOnClasspath(module, matches);
          }
        }
        ...
        if (null != file) {
          file = LogtalkFileUtil.getCanonicalFile(file);
        }
      ...
      private class Value extends XValue {
        ...
        // due to PSI errors in the logtalk PSI tree.
...
//                }
  ...
  private static String getRelativePath(Project project, VirtualFile file) {
    ...
    String packageName = LogtalkResolveUtil.getPackageName(psiFile);
