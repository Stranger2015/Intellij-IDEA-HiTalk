 ...
 */
public class LogtalkDebugLogger extends org.apache.log4j.Logger {

  private static final LogtalkDebugLoggerManager manager = new LogtalkDebugLoggerManager();
  private static final String FQCN = LogtalkDebugLogger.class.getName();

  public static interface Factory extends LoggerFactory {
    public LogtalkDebugLogger makeNewRootLoggerInstance();
  }

  public static class DefaultFactory implements Factory {
    public DefaultFactory() { }
    public LogtalkDebugLogger makeNewLoggerInstance(String name) {
      LogtalkDebugLogger logger = new LogtalkDebugLogger(name);
      return logger;
    }
    public LogtalkDebugLogger makeNewRootLoggerInstance() {
      LogtalkDebugLogger logger = new LogtalkDebugLogger("root");
      return logger;
    }
  }
  ...
  public static LogtalkDebugLogger getInstance() {
    return getLogger(getCallingClassName(), manager.getFactory());
  }
  @Deprecated @SuppressWarnings("deprecated")
  public static LogtalkDebugLogger getInstance(String name) {
    return getLogger(name, manager.getFactory());
  }
  @Deprecated @SuppressWarnings("deprecated")
  public static LogtalkDebugLogger getInstance(Class clazz) {
    return getLogger(clazz.getName(), manager.getFactory());
  }

  public static LogtalkDebugLogger getLogger() {
    return getLogger(getCallingClassName(), manager.getFactory());
  }
  public static LogtalkDebugLogger getLogger(String name) {
    return getLogger(name, manager.getFactory());
  }
  public static LogtalkDebugLogger getLogger(Class clazz) {
    return getLogger(clazz.getName(), manager.getFactory());
  }
  public static LogtalkDebugLogger getLogger(String name, LoggerFactory factory) {
    Logger logger = manager.getHierarchy().getLogger(name, factory);
    assert(logger instanceof LogtalkDebugLogger);
    ...
    return (LogtalkDebugLogger)logger;
  }
  ...
  private LogtalkDebugLogger(String name) {
    super(name);
  }
...
/**
 * Centralize global data for the LogtalkDebugLogger.
 */
class LogtalkDebugLoggerManager {
  private static final Level DEFAULT_LEVEL = Level.OFF;
  private Level defaultLevel = DEFAULT_LEVEL;

  private static final LogtalkDebugLogger.Factory DEFAULT_FACTORY = new LogtalkDebugLogger.DefaultFactory();
  private LogtalkDebugLogger.Factory configuredFactory = DEFAULT_FACTORY;
  ...
  public LogtalkDebugLoggerManager() {
 ...
 */
class HierarchyConfiguration implements LogtalkDebugLogger.HierarchyManipulator {
