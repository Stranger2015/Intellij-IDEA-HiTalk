...
public class LogtalkModuleUtil {
  ...
  private VirtualFile locateModuleFile(Module module, String haxeProjectPath) {
    ...
    VirtualFile projectFile = LogtalkFileUtil.locateFile(haxeProjectPath);
    if ( ! LogtalkFileUtil.isAbsolutePath(haxeProjectPath)) {
      if (null == projectFile) {
        ModuleRootManager rootMgr = ModuleRootManager.getInstance(module);
        projectFile = LogtalkFileUtil.locateFile(haxeProjectPath, rootMgr.getSourceRootUrls());
      }
      if (null == projectFile) {
        projectFile = LogtalkFileUtil.locateFile(haxeProjectPath, module.getModuleFilePath());
      }
    }
    return projectFile;
  }
