...
public class LogtalkProcessUtil {
  ...
  private LogtalkProcessUtil() {} // No instantiation.


  /**
   * Run a possibly interruptible process with the Logtalk SDK environment.  Checks
  ...
  public static int runProcess(List<String> command,
    ...
    if (null == timeLog && LOG.isDebugEnabled()) {
      timeLog = LogtalkDebugTimeLog.startNew("runProcess", LogtalkDebugTimeLog.Since.StartAndPrevious);
    ...
    try {
      ...
      ProcessBuilder builder = LogtalkSdkUtilBase.createProcessBuilder(command, fdir, sdkData);
      if (mixedOutput)
