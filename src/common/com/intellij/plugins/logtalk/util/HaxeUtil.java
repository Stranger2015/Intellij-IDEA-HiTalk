...
public class LogtalkUtil {
  public static void reparseProjectFiles(@NotNull final Project project) {
    Task.Backgroundable task = new Task.Backgroundable(project, LogtalkBundle.message("logtalk.project.reparsing"), false) {
        ...
        if (baseDir != null) {
          FileBasedIndex.getInstance().iterateIndexableFiles(new ContentIterator() {
            public boolean processFile(VirtualFile file) {
              if (LogtalkFileType.HAXE_FILE_TYPE == file.getFileType()) {
                haxeFiles.add(file);
              }
              return true;
            }
