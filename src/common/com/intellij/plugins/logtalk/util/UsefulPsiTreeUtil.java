...
public class UsefulPsiTreeUtil {
  ...
  private static void populateClassesList(List<LogtalkClass> classList, Project project, VirtualFile file) {
    VirtualFile[] files = file.getChildren();
    for (VirtualFile virtualFile : files) {
      if (virtualFile.getFileType().equals(LogtalkFileType.HAXE_FILE_TYPE)) {
        PsiFile psiFile = PsiManager.getInstance(project).findFile(virtualFile);

        String nameWithoutExtension = virtualFile.getNameWithoutExtension();

        List<LogtalkClass> haxeClassList = LogtalkResolveUtil.findComponentDeclarations(psiFile);
        for (LogtalkClass haxeClass : haxeClassList) {
  ...
  public static List<LogtalkClass> getClassesInDirectory(Project project, VirtualFile file) {
  ...
  @Nullable
  public static LogtalkPsiCompositeElement getChildOfType(@Nullable LogtalkPsiCompositeElement element, @Nullable IElementType elementType) {
    if (element == null) return null;
    for (LogtalkPsiCompositeElement child : PsiTreeUtil.getChildrenOfTypeAsList(element, LogtalkPsiCompositeElement.class)) {
