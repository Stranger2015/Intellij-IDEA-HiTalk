 ...
 */
public class LogtalkResolveTest extends MultiFileTestCase {
  @Override
  ...
  protected ModuleType getModuleType() {
    return LogtalkModuleType.getInstance();
  }
  ...
  private void doTest() throws Exception {
    doTest(new PerformAction() {
      ...
      public void performAction(VirtualFile rootDir, VirtualFile rootAfter) throws Exception {
        ...
        LogtalkFunctionDeclarationWithAttributes functionDeclarationWithAttributes =
          (LogtalkFunctionDeclarationWithAttributes)haxeFile.getClasses()[0].findMethodsByName("test", true)[0];

        assertNotNull(functionDeclarationWithAttributes);

        LogtalkBlockStatement statement = functionDeclarationWithAttributes.getBlockStatement();
        assert statement != null;
        LogtalkExpression expression = statement.getReturnStatementList().get(0).getExpression();
        assertNotNull(expression);

        LogtalkReferenceExpression referenceExpression = (LogtalkReferenceExpression)expression;
        PsiElement resolve = referenceExpression.resolve();

        assertNotNull(resolve);

        LogtalkFile haxeFile2 = (LogtalkFile)file2;
        assert haxeFile2 != null;
        LogtalkEnumDeclaration enumDeclaration = (LogtalkEnumDeclaration)haxeFile2.getClasses()[0];
        LogtalkEnumBody enumBody = enumDeclaration.getEnumBody();
